...
function readVectors(): Vectors {
  ...
  const vectorsPath = path.resolve(here, '../../../protocol-vectors/origin.json');
  return JSON.parse(fs.readFileSync(vectorsPath, 'utf8')) as Vectors;
}

test('normalizeOriginString matches shared vectors', () => {
  for (const vector of readVectors().normalize) {
    if (vector.expectError) {
      assert.equal(normalizeOriginString(vector.rawOriginHeader), null, vector.name);
    } else {
      assert.equal(normalizeOriginString(vector.rawOriginHeader), vector.normalizedOrigin, vector.name);
    }
  }
});

test('isOriginAllowed matches on normalized origin', () => {
  for (const vector of readVectors().allow) {
    assert.equal(
      isOriginAllowed(vector.rawOriginHeader, vector.allowedOrigins, vector.requestHost),
      vector.expectAllowed,
      vector.name,
    );
  }
});
