...
> The transport-independent framing that carries raw Ethernet frames between
> the browser and the proxy, over either WebSocket or a WebRTC data channel.
> The Rust crate `aero-l2-protocol` is the canonical implementation; the
> TypeScript and Go mirrors follow it.
...
For background and tradeoffs, see [`networking-architecture-rfc.md`](../areas/networking.md).
...
For Aero production deployments (the L2 tunnel networking decision):
...
- TypeScript (browser/client codec): `apps/web/src/shared/l2TunnelProtocol.ts`
  - Error code constants: `L2_TUNNEL_ERROR_CODE_*`
