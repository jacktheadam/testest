...
export class PersistentGpuCache {
  ...
  async putShader(key, value) {
    ...
    // Store the artifact whole.
    //
    // This used to pick out `wgsl` and `reflection` by name, which is the shape D3D9 translation
    // produces. D3D11's artifact carries `stage`, `bindings` and `vs_input_signature` instead, so
    // every one of its entries was written back missing the fields its reader requires, failed to
    // deserialise on the next lookup, and was deleted as corrupt — a cache that stored everything
    // and returned nothing. The reader is the only thing that knows the shape; this should not.
    const payload = { ...value };
    ...
    const record = {
      ...
      opfsFile,
      ...(storage === "idb" ? { payload } : {}),
      // Kept alongside the payload so the validity check and size accounting below can read it
      // without deserialising the whole artifact.
  ...
  async _loadShaderPayload(record) {
    ...
    // Records written before the artifact was stored whole carry no `payload`; fall back to the
    // two fields that shape had, so an existing cache keeps working rather than being discarded.
    return record.payload ?? { wgsl: record.wgsl, reflection: record.reflection };
  }
