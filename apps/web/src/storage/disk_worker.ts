    ...
    case "stat_disk": {
      ...
      if (meta.source === "local") {
        if (meta.backend === "opfs") {
          ...
          // The two backends have to agree on what this means, and they nearly did not.
          //
          // On OPFS a disk is one file truncated to its full length, so its size is the disk's
          // addressable size. On IndexedDB a disk is a sparse set of chunks — a blank disk stores
          // none at all, because absent chunks read as zeros — so summing them reports how much is
          // materialised, not how large the disk is. A freshly created 1 MiB blank disk therefore
          // came back as 1 MiB on one backend and 0 on the other.
          //
          // The sum still matters: an image whose chunks outrun its recorded size should not be
          // understated. Taking the larger of the two gives the addressable size in both cases.
          const db = await openDiskManagerDb();
          try {
            const materialisedBytes = await idbSumDiskChunkBytes(db, diskId);
            actualSizeBytes = Math.max(materialisedBytes, meta.sizeBytes ?? 0);
          } finally {
            db.close();
          }
        }
