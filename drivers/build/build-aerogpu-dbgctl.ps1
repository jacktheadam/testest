...
.DESCRIPTION
  ...
  `drivers/build/install-wdk.ps1`, imports a Visual Studio developer environment targeting x86
  ...
  `drivers/build/build-drivers.ps1`), the tool is copied into:
  ...
  so downstream `drivers/build/make-catalogs.ps1` stages it into `out/packages/**` and Guest Tools /
  driver bundle packaging can ship it.

.PARAMETER ToolchainJson
  Path to a toolchain manifest produced by `drivers/build/install-wdk.ps1` (default: out/toolchain.json).
#>
...
Remediation:
  - Re-run: pwsh -NoProfile -ExecutionPolicy Bypass -File drivers/build/install-wdk.ps1
...
Remediation:
  - Run: pwsh -NoProfile -ExecutionPolicy Bypass -File drivers/build/install-wdk.ps1
...
function Assert-PeMachineI386 {
  ...
  if ($machine -eq 0x8664) {
    throw ("AeroGPU dbgctl must be an x86 (32-bit) PE executable (IMAGE_FILE_MACHINE_I386 0x014c), but '{0}' is x64 (IMAGE_FILE_MACHINE_AMD64 0x8664). Ensure the dbgctl build uses the x86 toolchain/target, then re-run drivers/build/build-aerogpu-dbgctl.ps1." -f $absPath)
  }

  throw ("AeroGPU dbgctl must be an x86 (32-bit) PE executable (IMAGE_FILE_MACHINE_I386 0x014c), but '{0}' has an unexpected COFF Machine 0x{1:X4}. Ensure the dbgctl build uses the x86 toolchain/target, then re-run drivers/build/build-aerogpu-dbgctl.ps1." -f $absPath, $machine)
}
...
# Best-effort staging into the driver build output directories so downstream `drivers/build/make-catalogs.ps1`
...
foreach ($arch in @('x86', 'x64')) {
  ...
  if (-not (Test-Path -LiteralPath $driverOutDir -PathType Container)) {
    Write-Host "NOTE: not staging dbgctl into '$driverOutDir' (directory not found). Run drivers/build/build-drivers.ps1 first to produce out/drivers."
    continue
  }
