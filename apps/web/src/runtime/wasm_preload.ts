...
export async function precompileWasm(variant: WasmVariant): Promise<PrecompiledWasm> {
  ...
  const promise = (async () => {
    ...
    const module = await perf.spanAsync("wasm:compile", async () => {
      ...
      // (`/apps/web/src/...`) which `fetch()` cannot resolve (no base URL). Prefer reading the
      // bytes from disk.
      //
      ...
      if (isNodeEnv() && !/^https?:/i.test(url) && !/^data:/i.test(url)) {
        ...
        if (url.startsWith("file:")) {
          ...
          if (stripped.startsWith("/@fs/")) {
            ...
            // dev-server paths like `/apps/web/src/...`, so we keep a fallback below.
        ...
        // dev-server paths that are not directly readable from the filesystem (e.g. `/apps/web/src/...`).
