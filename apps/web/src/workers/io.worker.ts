...
function maybeInitHdaDevice(): void {
  ...
  try {
    ...
    //
    // See `wiki/areas/testing.md` for the Win7 audio smoke-test checklist and debugging
...
function runHdaMicCaptureTest(requestId: number): void {
  ...
  try {
    ...
    // Take SD#1 through the reset handshake before programming it. `SDnCTL.SRST` is not a start
    // bit: the descriptor is held in reset for as long as it reads 1, and the engine is only
    // readied on the 1 → 0 edge. Setting SRST alongside RUN parks the stream in reset, so it
    // never fetches a buffer descriptor and LPIB never advances.
    const SD_CTL_SRST = 1 << 0;
    const SD_CTL_RUN = 1 << 1;
    mmioWrite(sd1Base + 0x00, 4, SD_CTL_SRST);
    if ((mmioRead(sd1Base + 0x00, 4) & SD_CTL_SRST) === 0) {
      throw new Error("HDA SD#1 did not report entering reset (SDnCTL.SRST read back 0).");
    }
    mmioWrite(sd1Base + 0x00, 4, 0);
    if ((mmioRead(sd1Base + 0x00, 4) & SD_CTL_SRST) !== 0) {
      throw new Error("HDA SD#1 did not leave reset (SDnCTL.SRST read back 1).");
    }

    const ctl = (SD_CTL_RUN | (streamId << 20)) >>> 0; // RUN | STRM=2
