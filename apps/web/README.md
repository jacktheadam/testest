...
> The Vite app entrypoint in this directory (`apps/web/bringup.html`, `npm -w web run dev`) is
> legacy/experimental and is not used by CI/Playwright.

This directory contains shared runtime modules (`apps/web/src/...`) and the developer-facing WASM build
...
- `web/wddm-scanout-debug.html` — interactive WDDM scanoutState validation (guest RAM vs BAR1/VRAM backing, base_paddr/pitch/XRGB alpha forcing)
...
Each variant emits multiple wasm-pack packages under `apps/web/src/wasm/`:

- Core VM/runtime (`aero-wasm`):
  - **Single-threaded**: `apps/web/src/wasm/pkg-single/`
  - **Threaded** (shared memory + atomics): `apps/web/src/wasm/pkg-threaded/`
- GPU runtime (`aero-gpu-wasm`):
  - **Single-threaded**: `apps/web/src/wasm/pkg-single-gpu/`
  - **Threaded**: `apps/web/src/wasm/pkg-threaded-gpu/`
- Tier-1 compiler / JIT support (`aero-jit-wasm`):
  - **Single-threaded**: `apps/web/src/wasm/pkg-jit-single/`
  - **Threaded**: `apps/web/src/wasm/pkg-jit-threaded/`
  - Note: the runtime loader (`apps/web/src/runtime/jit_wasm_loader.ts`) intentionally prefers the
...
- `apps/web/src/wasm/*-dev/` (e.g. `pkg-single-dev/`, `pkg-threaded-gpu-dev/`)
...
- **Threaded/shared-memory WASM**: requires nightly `-Z build-std` and therefore uses the **pinned nightly**
  ...
  [Rust toolchain policy](wiki/decisions/0009-rust-toolchain-policy.md)).

## Build modes

Build orchestration lives in `apps/web/scripts/build_wasm.mjs`.
