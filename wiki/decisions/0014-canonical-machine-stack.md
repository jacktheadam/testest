# Canonical machine stack

**Decision (0014).** `aero-machine` is the canonical machine and VM stack, superseding the several partial VM stacks that accumulated before it.
...
- `aero-wasm::WasmVm` and `aero-wasm::WasmTieredVm`
  - Purpose: a **CPU-only** stepping loop for the browser **CPU worker runtime**
    (`apps/web/src/workers/cpu.worker.ts`, `vmRuntime=legacy`).
...
- `aero-wasm::Machine`
  ...
  - Used by:
    - `apps/web/src/workers/machine_cpu.worker.ts` (canonical machine runtime, `vmRuntime=machine`)
    - `apps/web/src/main.ts` (demo)
...
2. Export `aero-wasm::Machine` as the canonical browser-facing full-system VM API (done), and use it
   for standalone demos (`apps/web/src/main.ts`).
...
- [Canonical VM core](0008-canonical-vm-core.md) (canonical VM core crate)
