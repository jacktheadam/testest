...
export class RawWebGL2Presenter {
  ...
  constructor(canvas: HTMLCanvasElement | OffscreenCanvas, opts: any = {}) {
    ...
    // The annotation is doing real work. `canvas` is `HTMLCanvasElement | OffscreenCanvas`, and
    // the two declare `getContext` differently — the first through overloads keyed on the context
    // id, the second through one signature returning the whole union. Called on the union type,
    // TypeScript cannot apply the overload and widens the result to every context a canvas can
    // return, so every WebGL2 member access below fails. Asking for "webgl2" only ever yields a
    // `WebGL2RenderingContext`, which is what this says.
    const gl = canvas.getContext("webgl2", ctxAttrs) as WebGL2RenderingContext | null;
