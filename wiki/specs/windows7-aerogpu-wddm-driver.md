# Windows 7 AeroGPU WDDM driver

> The architecture of the AeroGPU Windows 7 driver stack: what the kernel-mode
> miniport must implement, the memory model it presents to the video memory
> manager, the present and scanout path, and the vblank and present-timing
> contract that keeps desktop composition stable.
>
> The device side of the same boundary — registers, ring, fences, allocation
> table — is specified in [aerogpu-device-abi.md](./aerogpu-device-abi.md).
...
`aerogpu-device-abi.md`).
  
---
 
## Why WDDM 1.1 (Win7) + what minimal success looks like
...
- `windows7-d3d-umd-ddi.md` (D3D11 `d3d11umddi.h` function-table checklist: REQUIRED vs stubbable for a crash-free FL10_0 skeleton)
...
## High-level architecture (guest ↔ emulator boundary)
...
## KMD responsibilities and required DxgkDdi entrypoints (Win7 WDDM 1.1)
...
- **AeroGPU MVP behavior:** Accept only:
  ...
  - A small whitelist of modes and pixel formats (32bpp scanout-compatible):
    ...
    - `D3DDDIFMT_A8R8G8B8` (byte-layout compatible; alpha ignored / treated as opaque by scanout)
...
- **AeroGPU MVP behavior:** Set/clear `AEROGPU_MMIO_REG_SCANOUT0_ENABLE` (1/0) as a visibility toggle. When cleared, the device model blanks output (stopping vblank pacing) but keeps WDDM scanout ownership sticky (legacy VGA/VBE remains suppressed until VM reset).
...
- **AeroGPU MVP behavior:**
  ...
  - Use WDDM allocation private driver data (`aerogpu_wddm_alloc_priv` in `drivers/aerogpu/protocol/aerogpu_wddm_alloc.h`) to carry stable per-allocation IDs across the UMD↔KMD boundary and across processes.
    ...
    - For shared allocations, dxgkrnl preserves and replays the blob on `OpenResource`/`OpenAllocation` so all processes observe identical IDs. Do **not** use the numeric value of the user-mode shared `HANDLE` as a stable protocol key: for real NT handles it is process-local (commonly different in a consumer after `DuplicateHandle`), and some stacks use token-style shared handles that still must not be treated as stable IDs (and should not be passed to `CloseHandle`).
  - The KMD stores the IDs in its allocation bookkeeping and uses `alloc_id` when building the per-submit allocation table.
  - See `aerogpu-device-abi.md` for the end-to-end shared-surface `share_token` contract and Win7 guest validation tests.
...
- [`win7-d3d11-map-unmap.md`](windows7-d3d-umd-ddi.md)
...
See `drivers/aerogpu/kmd/README.md` and `windows7-aerogpu-validation.md` for tuning guidance.
...
- The command stream references guest backing memory via `backing_alloc_id` (a stable `alloc_id`); see [`aerogpu-device-abi.md`](aerogpu-device-abi.md).
...
## Present + scanout path (single output)
...
- BAR1 (`AEROGPU_PCI_BAR1_INDEX`, index 1): prefetchable MMIO VRAM aperture (`AEROGPU_PCI_BAR1_SIZE_BYTES` is currently 64 MiB). This is primarily used for legacy VGA/VBE compatibility and is outside the current Win7 WDDM memory model (the in-tree driver treats the adapter as system-memory-backed).
...
- **Scanout 0** (if `AEROGPU_FEATURE_SCANOUT` is set)
  ...
  - `AEROGPU_MMIO_REG_SCANOUT0_PITCH_BYTES`
  - `AEROGPU_MMIO_REG_SCANOUT0_FB_GPA_LO/HI`
  - Format semantics (important for correct presentation): X8 formats are fully opaque and `*_SRGB` formats are layout-identical to UNORM but differ in interpretation; see `aerogpu-device-abi.md` §2.5.1.
...
- **Cursor (optional)**
  - If `AEROGPU_FEATURE_CURSOR` is set:
    ...
    - Format semantics are the same as scanout (X8 opaque; `*_SRGB` interpretation rules); see `aerogpu-device-abi.md` §2.5.1.
...
## Toolchain / build / signing (Windows 7 SP1)
...
Recommended (matches CI, pins tool versions): follow `../areas/windows-drivers.md`:
...
See `drivers/aerogpu/README.md` for driver layout/entrypoints and `../areas/windows-drivers.md` for the current packaging/signing workflow.
...
## Validation strategy (minimal acceptance tests)
...
## Clean-room / licensing constraints
...
#### Vblank enable/disable semantics
...
#### WaitForVerticalBlankEvent semantics
...
#### Present + SyncInterval behavior (minimum viable)
...
#### Occlusion / minimization / idle desktop
