#![deny(unsafe_code)]
//! CPU rasterizer for the AeroGPU command stream.
//!
//! The other backends hand the command stream to a real graphics API. This one executes it
//! outright — vertex fetch, triangle setup, depth test, texture sampling, blend — against plain
//! memory, so it needs no adapter, no driver, and no display.
//!
//! That makes it the backend that always runs. Tests that depend on a GPU skip when there is no
//! adapter, and a suite which skips its hardest cases reports the same green as one that passed
//! them; this backend gives the command stream a home where behaviour is decided by the code in
//! this repository rather than by whatever the host happens to provide.
...
use aero_devices_gpu::regs::{AeroGpuRegs, AerogpuErrorCode};
use aero_devices_gpu::ring::AeroGpuSubmitDesc;
use aero_devices_gpu::AeroGpuFormat;

/// Wire structs that may be reconstructed from a little-endian byte prefix.
///
/// The AeroGPU command stream arrives as untrusted guest bytes, and every packet begins with a
/// fixed `#[repr(C)]` header of integer fields. Reconstituting one is a byte copy, but only for
/// types where *every* bit pattern is a valid value — integers and arrays of them. A `Copy` bound
/// does not say that: `bool`, `char`, and enums are all `Copy` and all have invalid bit patterns.
///
/// This trait carries the missing promise. It is implemented only for the protocol structs below,
/// each of which is `#[repr(C)]` with integer fields exclusively, which is what makes
/// [`read_packed_prefix`] sound.
///
/// Fields are read in host byte order and byte-swapped by the caller with `u32::from_le` and
/// friends, matching how the rest of the AeroGPU stack handles the wire format.
trait WireStruct: Copy {}

impl WireStruct for cmd::AerogpuCmdClear {}
impl WireStruct for cmd::AerogpuCmdCopyBuffer {}
impl WireStruct for cmd::AerogpuCmdCopyTexture2d {}
impl WireStruct for cmd::AerogpuCmdCreateBuffer {}
impl WireStruct for cmd::AerogpuCmdCreateSampler {}
impl WireStruct for cmd::AerogpuCmdCreateTexture2d {}
impl WireStruct for cmd::AerogpuCmdDestroyInputLayout {}
impl WireStruct for cmd::AerogpuCmdDestroyResource {}
impl WireStruct for cmd::AerogpuCmdDestroySampler {}
impl WireStruct for cmd::AerogpuCmdDestroyShader {}
impl WireStruct for cmd::AerogpuCmdDraw {}
impl WireStruct for cmd::AerogpuCmdDrawIndexed {}
impl WireStruct for cmd::AerogpuCmdExportSharedSurface {}
impl WireStruct for cmd::AerogpuCmdImportSharedSurface {}
impl WireStruct for cmd::AerogpuCmdReleaseSharedSurface {}
impl WireStruct for cmd::AerogpuCmdResourceDirtyRange {}
impl WireStruct for cmd::AerogpuCmdSetDepthStencilState {}
impl WireStruct for cmd::AerogpuCmdSetIndexBuffer {}
impl WireStruct for cmd::AerogpuCmdSetInputLayout {}
impl WireStruct for cmd::AerogpuCmdSetPrimitiveTopology {}
impl WireStruct for cmd::AerogpuCmdSetRasterizerState {}
impl WireStruct for cmd::AerogpuCmdSetRenderTargets {}
impl WireStruct for cmd::AerogpuCmdSetSamplers {}
impl WireStruct for cmd::AerogpuCmdSetScissor {}
impl WireStruct for cmd::AerogpuCmdSetTexture {}
impl WireStruct for cmd::AerogpuCmdSetViewport {}
impl WireStruct for cmd::AerogpuInputLayoutBlobHeader {}
impl WireStruct for cmd::AerogpuInputLayoutElementDxgi {}

/// Reconstruct a wire struct from the start of `buf`, or `None` if `buf` is too short.
fn read_packed_prefix<T: WireStruct>(buf: &[u8]) -> Option<T> {
    let size = core::mem::size_of::<T>();
    if buf.len() < size {
        return None;
    }

    // SAFETY: `WireStruct` is implemented only for `#[repr(C)]` structs whose fields are integers
    // and arrays of integers, so every bit pattern of `size` bytes is a valid `T`. The length is
    // checked above, and `read_unaligned` imposes no alignment requirement on `buf`.
    #[allow(unsafe_code)]
...
impl AeroGpuSoftwareExecutor {
    ...
    fn parse_input_layout_blob(blob: &[u8]) -> ParsedInputLayout {
        ...
        let hdr = match read_packed_prefix::<cmd::AerogpuInputLayoutBlobHeader>(blob) {
            Some(v) => v,
        ...
        for _ in 0..element_count {
            let elem = match read_packed_prefix::<cmd::AerogpuInputLayoutElementDxgi>(&blob[off..])
            {
                Some(v) => v,
                None => return ParsedInputLayout::default(),
            };
    ...
    fn dispatch_cmd(
        ...
        match op {
            ...
            cmd::AerogpuCmdOpcode::CreateBuffer => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdCreateBuffer>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::CreateTexture2d => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdCreateTexture2d>(packet)
                {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
            ...
            cmd::AerogpuCmdOpcode::DestroyResource => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdDestroyResource>(packet)
                {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
            ...
            cmd::AerogpuCmdOpcode::ResourceDirtyRange => {
                let packet_cmd =
                    match read_packed_prefix::<cmd::AerogpuCmdResourceDirtyRange>(packet) {
                        Some(v) => v,
                        None => {
                            Self::record_error(regs);
                            return false;
            ...
            cmd::AerogpuCmdOpcode::CopyBuffer => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdCopyBuffer>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::CopyTexture2d => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdCopyTexture2d>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::DestroyShader => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdDestroyShader>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::DestroyInputLayout => {
                let packet_cmd =
                    match read_packed_prefix::<cmd::AerogpuCmdDestroyInputLayout>(packet) {
                        Some(v) => v,
                        None => {
                            Self::record_error(regs);
                            return false;
            ...
            cmd::AerogpuCmdOpcode::SetInputLayout => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdSetInputLayout>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::SetRasterizerState => {
                let packet_cmd =
                    match read_packed_prefix::<cmd::AerogpuCmdSetRasterizerState>(packet) {
                        Some(v) => v,
                        None => {
                            Self::record_error(regs);
                            return false;
            ...
            cmd::AerogpuCmdOpcode::SetDepthStencilState => {
                let packet_cmd =
                    match read_packed_prefix::<cmd::AerogpuCmdSetDepthStencilState>(packet) {
                        Some(v) => v,
                        None => {
                            Self::record_error(regs);
                            return false;
            ...
            cmd::AerogpuCmdOpcode::SetTexture => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdSetTexture>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::CreateSampler => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdCreateSampler>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::DestroySampler => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdDestroySampler>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::SetSamplers => {
                ...
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdSetSamplers>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::SetRenderTargets => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdSetRenderTargets>(packet)
                {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
            ...
            cmd::AerogpuCmdOpcode::SetViewport => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdSetViewport>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::SetScissor => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdSetScissor>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::SetIndexBuffer => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdSetIndexBuffer>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::SetPrimitiveTopology => {
                let packet_cmd =
                    match read_packed_prefix::<cmd::AerogpuCmdSetPrimitiveTopology>(packet) {
                        Some(v) => v,
                        None => {
                            Self::record_error(regs);
                            return false;
            ...
            cmd::AerogpuCmdOpcode::Clear => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdClear>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::Draw => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdDraw>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::DrawIndexed => {
                let packet_cmd = match read_packed_prefix::<cmd::AerogpuCmdDrawIndexed>(packet) {
                    Some(v) => v,
                    None => {
                        Self::record_error(regs);
                        return false;
            ...
            cmd::AerogpuCmdOpcode::ExportSharedSurface => {
                let packet_cmd =
                    match read_packed_prefix::<cmd::AerogpuCmdExportSharedSurface>(packet) {
                        Some(v) => v,
                        None => {
                            Self::record_error(regs);
                            return false;
            ...
            cmd::AerogpuCmdOpcode::ImportSharedSurface => {
                let packet_cmd =
                    match read_packed_prefix::<cmd::AerogpuCmdImportSharedSurface>(packet) {
                        Some(v) => v,
                        None => {
                            Self::record_error(regs);
                            return false;
            ...
            cmd::AerogpuCmdOpcode::ReleaseSharedSurface => {
                let packet_cmd =
                    match read_packed_prefix::<cmd::AerogpuCmdReleaseSharedSurface>(packet) {
                        Some(v) => v,
                        None => {
                            Self::record_error(regs);
                            return false;
...
mod tests {
    ...
    use aero_devices_gpu::regs::irq_bits;
    use memory::Bus;
