...
> The TypeScript layer that loads the emulator's WebAssembly, wires it to
> browser APIs, and coordinates execution across Web Workers sharing memory.
> Because shared memory requires a cross-origin-isolated context, the whole
> host is built around the COOP/COEP deployment contract.

## What it is

Aero's emulator core is Rust compiled to WebAssembly; the **web host** is the
TypeScript layer that loads that WASM, wires it to browser APIs, and
coordinates execution across Web Workers sharing `SharedArrayBuffer`. Because
shared memory is only available in a cross-origin-isolated context, the whole
host is built around the COOP/COEP deployment contract (§3).

There is one browser application, `apps/web/`, with two Vite configurations
over it:

- **The harness** (`vite.harness.config.ts`, served by `pnpm run dev` or
  `pnpm run dev:harness`) — the canonical entry, and what Playwright drives.
- **The application config** (`apps/web/vite.config.ts`) — used for the fuller
  manual UI, including the disks and workers panels and the smoke pages.

These were once two separate applications at the repository root and under
`apps/web/`; they were merged, and the surviving difference is two *shells* over one
module library. See [the repo-layout decision](../decisions/0001-repo-layout.md).

> **Known defect:** the `dev:web` and `serve:coi:web` scripts in the root
> `package.json` still pass `--config web/vite.config.ts` and a `web` root, both
> of which were removed in the merge. They fail if invoked; the live config is
> `apps/web/vite.config.ts`. Several other root scripts have the same problem —
> see the toolchain defects in
> [build-and-tooling.md](./build-and-tooling.md#known-toolchain-defects).

Independently, the worker runtime has two CPU-side modes (the canonical machine stack decision,
`wiki/decisions/0014-canonical-machine-stack.md`):

- `vmRuntime=legacy` — CPU worker drives the CPU-only WASM exports
  (`WasmVm` / `WasmTieredVm`) via `apps/web/src/workers/cpu.worker.ts`; guest
  device models live in the I/O worker.
- `vmRuntime=machine` — CPU worker runs the canonical
  `aero_machine::Machine` via `apps/web/src/workers/machine_cpu.worker.ts` and
  owns guest devices; the I/O worker runs host-only (no guest device
  models). WebUSB/WebHID passthrough and guest audio via the worker stack
  are **not yet available** in this mode.

The WASM entrypoint is `aero_wasm.js`, built from `crates/aero-wasm`
(exports include `WasmVm`, `WasmTieredVm`, and the canonical `Machine`;
verified in `crates/aero-wasm/src/lib.rs`).

## Worker topology and shared memory

Main thread: UI, input capture, and a **coordinator**
(`apps/web/src/runtime/coordinator.ts`, protocol in `apps/web/src/runtime/protocol.ts`)
that owns worker lifecycle and IPC routing. Workers (all under
`apps/web/src/workers/`): CPU (`cpu.worker.ts` / `machine_cpu.worker.ts`), GPU
(`gpu.worker.ts`), I/O (`io.worker.ts`), JIT (`jit.worker.ts`).

Because wasm32 linear memory is ≤ 4 GiB and browsers cannot reliably allocate
a single multi-GiB `SharedArrayBuffer`, Aero uses **split buffers** (the shared memory layout decision,
`wiki/decisions/0003-shared-memory-layout.md`). The canonical allocation
(`apps/web/src/runtime/shared_layout.ts`):

| Buffer | Type | Contents |
|---|---|---|
| `guestMemory` | shared `WebAssembly.Memory` | Guest physical RAM (256 MiB minimum; presets 256/512/1024/2048/3072 MiB, best-effort). Guest paddr 0 is at a runtime-published `guest_base` offset, not linear 0. |
| `controlSab` | `SharedArrayBuffer` | Status block + per-worker command/event rings |
| `ioIpcSab` | `SharedArrayBuffer` | High-frequency CPU↔I/O device IPC rings (AIPC) |
| `vram` | `SharedArrayBuffer` | AeroGPU BAR1/VRAM aperture backing; mapped in the PCI MMIO window at `PCI_MMIO_BASE = 0xE000_0000` (`apps/web/src/arch/guest_phys.ts`) |
| `sharedFramebuffer` | `SharedArrayBuffer` | Legacy/demo CPU→GPU RGBA framebuffer |
| `scanoutState` | `SharedArrayBuffer` | Shared scanout descriptor (legacy text/VBE ↔ WDDM handoff), seqlock-style publish |
| `cursorState` | `SharedArrayBuffer` | Shared hardware cursor descriptor |

Key contracts:

- Guest physical→linear translation is piecewise (low RAM below the ECAM/PCI
  MMIO hole, high RAM remapped above 4 GiB); enforced by
  `apps/web/src/arch/guest_ram_translate.ts`.
- IPC is a binary protocol over lock-free SPSC/MPSC ring buffers, not
  `postMessage` JSON: spec `wiki/areas/platform-and-firmware.md`, Rust `crates/aero-ipc`,
  TS `apps/web/src/ipc`. Ring header is `Int32Array[4]`
  (`head`, `tail_reserve`, `tail_commit`, `capacity`); records are
  length-prefixed, 4-byte aligned, with a `0xFFFF_FFFF` wrap marker.
- `Atomics.wait()` is forbidden on the main thread; the coordinator uses
  `Atomics.waitAsync()` where available, else polls. Workers `Atomics.notify()`
  after mutating shared queues.
- Power/reset: guest ACPI S5/reset requests surface to the coordinator as
  `acpi_power_off_request` / `acpi_reset_request` messages so it can stop or
  reset workers, flush storage, and update UI; host-side force
  power-off/reset controls call the same entry points.
- Snapshots: coordinator broadcasts save/restore; workers produce versioned,
  deterministic byte blobs (sorted iteration, explicit TLV-ish formats);
  host-side resources (sockets, OPFS handles) are treated as reconnectable,
  not bit-restorable.

## Cross-origin isolation & security headers

This is a **hard deployment contract**, verified by tests
(`tests/e2e/security-headers.spec.ts`, `tests/e2e/csp-fallback.spec.ts`):

- Required on the HTML document *and* every JS/WASM/worker response
  (including 304s):
  `Cross-Origin-Opener-Policy: same-origin`,
  `Cross-Origin-Embedder-Policy: require-corp`.
- Recommended hardening: `Cross-Origin-Resource-Policy: same-origin`,
  `Origin-Agent-Cluster: ?1`.
- CSP (canonical value in `scripts/headers.json`):
  ...
  - `'wasm-unsafe-eval'` (not `'unsafe-eval'`) is required because Aero
    compiles WASM at runtime (module load and the Tier-1/2 JIT). Without it,
    `WebAssembly.compile`/`instantiate` are blocked in Chromium, Firefox,
    and WebKit alike (validated by the PoC + Playwright coverage).
  - The `aero-gateway.invalid` connect-src entries are documentation
    placeholders; replace per deployment or drop.
  - Baseline extras: `Referrer-Policy: no-referrer`,
    `X-Content-Type-Options: nosniff`,
    `Permissions-Policy: camera=(), geolocation=(), microphone=(self), usb=(self)`
    (the `usb` directive gates WebUSB).
- Canonical header values live in `scripts/headers.json` +
  `scripts/security_headers.mjs` and are consumed by `vite.harness.config.ts`,
  `apps/web/vite.config.ts`, and the gateway middleware
  (`services/gateway/src/middleware/crossOriginIsolation.ts`,
  `securityHeaders.ts`). `apps/web/public/_headers` carries the same values for
  static hosts that honor `_headers` (copied into `dist/` by the harness
  build).
- Capability fallback: when COOP/COEP is unavailable, the runtime can fall
  back to a non-shared-memory single-threaded WASM build (the WebAssembly build variants decision); when CSP
  forbids dynamic compilation, the host exposes `jit_dynamic_wasm: false`
  (`src/platform/features.ts`, `apps/web/src/platform/features.ts`) and must not
  enable the JIT tiers. `VITE_DISABLE_COOP_COEP=1` disables the headers on
  dev/preview servers to exercise the fallback.
- PoC app for CSP/COOP/COEP behavior: `apps/web/public/wasm-jit-csp/`; the local
  server that used to serve its variants was removed with the `server/`
  purge (2026-07-23).
- Common pitfalls: missing headers on any subresource keeps
  `crossOriginIsolated === false`; COEP blocks non-opted-in cross-origin
  subresources (self-host assets); secure context is required
  (`https://` or `http://localhost`).

**Disk byte streaming under COEP:** prefer serving disk bytes same-origin
(e.g. reverse-proxy under `/disk/...`). Cross-origin requires CORS (opaque
`no-cors` responses are useless to `fetch()`), CORP on the responses, an
`OPTIONS` preflight allowing `Range`, exposed `Content-Range`, and
`Cache-Control: no-transform` so proxies/CDNs never recompress bytes.

**Historical:** the per-provider deployment templates (`netlify.toml`,
`vercel.json`, `deploy/{caddy,nginx,k8s,...}`) and the static
`check:security-headers` script were removed with the CI/deploy purge
(2026-07-23). Runtime header regression coverage remains:
`pnpm run test:security-headers`.

## Browser API usage (current truth)

| API | Status in Aero |
|---|---|
| WebAssembly (+ SIMD) | Required. wasm32 build; guest RAM lives in a shared `WebAssembly.Memory`. |
| WASM threads / `SharedArrayBuffer` / `Atomics` | Required for the threaded build (the WebAssembly build variants decision); single-thread fallback exists. Threaded build needs pinned nightly + `rust-src` (`scripts/toolchains.json`, `rust.nightlyWasm`) and the internal `wasm-threaded` cargo feature. |
| Web Workers | Required (module workers; CSP `worker-src 'self' blob:`). |
| WebGPU | Preferred graphics backend (Rust `wgpu`, `crates/aero-webgpu`). Optional features negotiated opportunistically: `texture-compression-bc/etc2/astc`, `float32-filterable`, `timestamp-query` (profiling only). `AERO_DISABLE_WGPU_TEXTURE_COMPRESSION=1` forces CPU-decompression paths; compression features are also avoided on the wgpu GL backend due to correctness issues. |
| WebGL2 | Fallback presenter when `navigator.gpu` is absent. Subset: no compute; must handle the Y-flip (GL bottom-left origin) and loose sRGB expectations. |
| OffscreenCanvas | Preferred GPU presenter model (canvas transferred to the GPU worker); main-thread canvas fallback for browsers lacking worker OffscreenCanvas (Safari). |
| OPFS | Required for the synchronous disk path: `crates/aero-opfs` (`OpfsByteStorage`) uses `createSyncAccessHandle` in a worker. |
| IndexedDB | Small persistent caches only — async-only, cannot back the synchronous Rust disk stack (see `crates/st-idb`). Concrete user: GPU derived-artifact cache `apps/web/gpu-cache/persistent_cache.ts` (DB `aero-gpu-cache`, stores `shaders`/`pipelines`, optional OPFS spill; cached WGSL is untrusted and revalidated before use). |
| AudioWorklet | Required for audio output. Playback ring contract: 16-byte `u32[4]` header (`readFrameIndex`, `writeFrameIndex`, `underrunCount`, `overrunCount` — monotonic wrapping frame counters) + interleaved `f32` frames; canonical layout `apps/web/src/platform/audio_worklet_ring_layout.js`, mirrored in Rust at `crates/platform/src/audio/worklet_bridge.rs`. Entry helper `createAudioOutput` (`apps/web/src/platform/audio.ts`) handles `webkitAudioContext`, option-set fallbacks, `startupPrefillFrames`, and `discardOnResume` (posts `ring.reset` on suspend→running). Worklet asset is emitted manually by both Vite configs (Vite does not chase `addModule` ESM imports). Always size buffers from the *actual* `audioContext.sampleRate`. |
| WebSocket | All guest TCP egress goes to the Aero Gateway (`services/gateway`): `GET /tcp?v=1&host=&port=` appended to the gateway base URL; token auth rides as a WebSocket subprotocol (browsers can't set handshake headers). |
| WebRTC | UDP egress via the UDP relay (`proxy/webrtc-udp-relay`, protocol in `proxy/webrtc-udp-relay/PROTOCOL.md`): v1 frames are legacy IPv4-only; v2 frames start with `0xA2 0x02` and support IPv4/IPv6. UDP uses an unreliable/unordered DataChannel; the L2 tunnel (the L2 tunnel networking decision, `wiki/decisions/0013-networking-l2-tunnel.md`) requires reliable+ordered channels. |
| Pointer Lock / Fullscreen / Keyboard Lock | Input capture. Keyboard Lock is best-effort, requires transient user activation from the same gesture as pointer lock, and needs an alternate capture-exit path if `Escape` is locked. Global hotkey handlers must respect `event.defaultPrevented` so VM capture can swallow keys. |
| Gamepad | Optional; mapped into HID gamepad reports (see §Input passthrough). |
| WebUSB / WebHID / WebSerial | Optional, Chromium-only; see below. |
| WebCodecs | Optional. |

## Input passthrough (WebUSB / WebHID)

Chromium-only, secure-context, and **main-thread-only** (`requestDevice`
needs transient user activation; user activation does not cross
`postMessage`). Currently implemented for `vmRuntime=legacy` only.

- Production pattern is "main thread brokers, worker does I/O": `UsbBroker`
  on the main thread + `WebUsbPassthroughRuntime` in the I/O worker speaking
  a `MessagePort` protocol — `apps/web/src/usb/{usb_broker,webusb_passthrough_runtime,usb_proxy_protocol}.ts`.
- When cross-origin isolation and SAB/Atomics are available, an optional ring
  fast path is negotiated via `usb.ringAttach`
  (`apps/web/src/usb/usb_proxy_ring.ts`): worker→main `UsbHostAction` ring and
  main→worker `UsbHostCompletion` ring; falls back to typed `postMessage`
  (`usb.action`/`usb.completion`) when rings are unavailable/full, and either
  side can disable the fast path with `usb.ringDetach` on decode corruption.
  WebHID mirrors this with `hid.ring.init`/`hid.ringAttach`/`hid.ringDetach`
  (`apps/web/src/hid/`).
- `USBDevice` structured-clone support is browser-dependent — probe at
  runtime (`apps/web/src/usb/webusb_panel.ts`, `/webusb_diagnostics.html`); a
  `DataCloneError` means "keep WebUSB on the main thread and proxy".
- WebHID does **not** expose raw report descriptors; Aero synthesizes
  byte-accurate descriptors for the guest from the structured view.
- The legacy repo-root demo WebUSB RPC (`src/platform/legacy/webusb_*`) was
  removed; the `UsbHostAction` contract above is canonical (the canonical USB stack decision,
  `wiki/decisions/0015-canonical-usb-stack.md`; controller/device models:
  `crates/aero-usb`).
- Shared test vectors for the passthrough/HID contracts live in
  `protocol-vectors/*.json` and are consumed by Rust tests (e.g.
  `crates/aero-usb/tests/`, `crates/aero-devices-input/tests/`). **These are
  load-bearing data, not documentation** — relocated out of `docs/` ahead of
  the docs purge.

## Backend & proxy configuration (env)

Canonical build/test variables are in [testing.md](./testing.md); the
deployment-side surface (consumed by `services/gateway`,
`proxy/webrtc-udp-relay`, `crates/aero-l2-proxy`):

- Gateway→relay metadata: `UDP_RELAY_BASE_URL`, `UDP_RELAY_AUTH_MODE`
  (`none`/`api_key`/`jwt`), `UDP_RELAY_API_KEY`, `UDP_RELAY_JWT_SECRET`,
  `UDP_RELAY_TOKEN_TTL_SECONDS` (default 300), `UDP_RELAY_AUDIENCE`,
  `UDP_RELAY_ISSUER`.
- UDP relay: `AERO_STUN_URLS`, `AERO_TURN_URLS`, `UDP_BINDING_IDLE_TIMEOUT`
  (60s), `UDP_INBOUND_FILTER_MODE` (default `address_and_port`; `any` is
  full-cone, less safe), `UDP_REMOTE_ALLOWLIST_IDLE_TIMEOUT`,
  `MAX_ALLOWED_REMOTES_PER_BINDING` (1024), DoS caps
  `WEBRTC_DATACHANNEL_MAX_MESSAGE_BYTES`,
  `WEBRTC_SCTP_MAX_RECEIVE_BUFFER_BYTES`, `WEBRTC_SESSION_CONNECT_TIMEOUT`
  (30s).
- L2 tunnel proxy (`crates/aero-l2-proxy`): listen/Origin/Host validation
  (`AERO_L2_PROXY_LISTEN_ADDR` default `0.0.0.0:8090`,
  `AERO_L2_ALLOWED_ORIGINS`, `AERO_L2_ALLOWED_HOSTS`,
  `AERO_L2_TRUST_PROXY_HOST`), auth (`AERO_L2_AUTH_MODE`, `AERO_L2_API_KEY`,
  `AERO_L2_SESSION_SECRET`, `AERO_L2_JWT_SECRET`/`_AUDIENCE`/`_ISSUER`,
  escape hatches `AERO_L2_OPEN` / `AERO_L2_INSECURE_ALLOW_NO_AUTH`),
  connection/rate/byte caps (`AERO_L2_MAX_CONNECTIONS` default 64,
  `..._PER_SESSION`, `..._PER_IP`, `AERO_L2_MAX_BYTES_PER_CONNECTION`,
  `AERO_L2_MAX_FRAMES_PER_SECOND`, `AERO_L2_MAX_FRAME_PAYLOAD` default 2048,
  `AERO_L2_MAX_CONTROL_PAYLOAD` default 256), egress policy
  (`AERO_L2_ALLOW_PRIVATE_IPS`, `AERO_L2_ALLOWED_{TCP,UDP}_PORTS`,
  `AERO_L2_ALLOWED_DOMAINS`, `AERO_L2_BLOCKED_DOMAINS`), PCAPNG capture
  (`AERO_L2_CAPTURE_DIR`, `AERO_L2_CAPTURE_MAX_BYTES` default 64 MiB,
  `AERO_L2_CAPTURE_FLUSH_INTERVAL_MS`), liveness (`AERO_L2_PING_INTERVAL_MS`,
  `AERO_L2_IDLE_TIMEOUT_MS`, `AERO_L2_TRUST_PROXY`).
- Full normative table with defaults and consumers: `wiki/areas/testing.md`;
  L2 runbook: `wiki/areas/networking.md`.

## Contributor security hygiene

(from `security.md`, complements root `SECURITY.md`)

- Never commit secrets; use `*.env.example` templates (`.env` is gitignored).
  Also never commit `.npmrc`/`.netrc`/key material (`*.pem`/`*.key`/`*.p12`/
  `*.pfx`/`*.tfvars`).
- A leaked secret is compromised: rotate first, assess blast radius, rewrite
  history only secondarily (`git filter-repo`), notify per `SECURITY.md`.
- CodeQL/secret-scanning automation ran under `.github/` — **removed with
  the CI purge** (historical until CI is re-established). The triage
  doctrine (fix preferred; dismiss with justification; targeted inline
  `codeql[...]` suppressions, never broad suite disables) still applies.

## Development environment

- Node: pinned in `.nvmrc` (24.18.0); enforced by
  `scripts/check-node-version.mjs` (`AERO_ALLOW_UNSUPPORTED_NODE=1` downgrades
  the hard error to a warning; `AERO_CHECK_NODE_QUIET=1` silences the
  newer-than-pinned note).
- Rust: stable pinned in `rust-toolchain.toml`; threaded-WASM nightly pinned
  in `scripts/toolchains.json` (`rust.nightlyWasm`) with `wasm32-unknown-unknown`
  + `rust-src`. `just setup` installs toolchains and runs the pnpm install.
- Other tools: `wasm-pack`, binaryen (`wasm-opt`), QEMU + `mtools` (boot
  tests), `iasl` (ACPI), Playwright browsers via
  `node scripts/playwright_install.mjs chromium`.
- **Historical:** the dev-container definition (`.devcontainer/`) was
  removed with the CI/deploy purge (2026-07-23); `build-and-tooling.md`
  Option A no longer applies. No Nix flake exists [aspirational].

## Open issues / debt

- `package.json` workspaces and `pnpm run test:unit` still reference the
  purged `server/` workspace — expected to be resolved as the purge lands
  [unverified].
- `vmRuntime=machine` gaps: no WebUSB/WebHID passthrough, no guest audio via
  the browser worker stack (audio checklist applies to `vmRuntime=legacy`).
- WebGPU adapter/limits variability in headless CI is handled by gating, not
  by guaranteed availability — see [testing.md](./testing.md).
- `Atomics.waitAsync` is not universal; the coordinator's polling fallback
  adds latency to worker→main wakeups.

## Pointers

- Architecture context & ADRs: `wiki/decisions/` (0002 isolation, 0003 memory
  layout, 0004 build variants, 0013 L2 tunnel, 0014 machine stack, 0015 USB
  stack); IPC spec `wiki/areas/platform-and-firmware.md`.
- Machine/device side: `crates/aero-machine`, `crates/aero-wasm`,
  `wiki/areas/platform-and-firmware.md`.
- Testing the host: [testing.md](./testing.md); GPU backends
  `wiki/areas/graphics.md`; audio `wiki/areas/audio.md`.
...
The high-performance (threaded) build requires **cross-origin isolation** to enable `SharedArrayBuffer` and WASM threads (see [Cross-origin isolation](../decisions/0002-cross-origin-isolation.md) and [WebAssembly build variants](../decisions/0004-wasm-build-variants.md)).
...
- **Canonical stack selection:** for the browser runtime, the canonical USB stack is `aero-usb` + `aero-wasm` + `apps/web/`; see [Canonical USB stack](../decisions/0015-canonical-usb-stack.md).
- **UHCI passthrough mapping:** for the guest UHCI TD ↔ WebUSB transfer mapping (including “TD-level NAK while pending”), see [`usb-and-input.md`](usb-and-input.md).
- **Troubleshooting:** for `requestDevice()` / `open()` / `claimInterface()` failures (protected interface classes, WinUSB/udev permissions, etc.), see [`usb-and-input.md`](usb-and-input.md).
...
- **B) Main thread proxies all WebUSB I/O (fallback):**
  ...
  - In the production web runtime (`apps/web/`), this is implemented by `UsbBroker` (main thread) + `WebUsbPassthroughRuntime` (worker) using a `MessagePort` protocol (`apps/web/src/usb/{usb_broker,webusb_passthrough_runtime,usb_proxy_protocol}.ts`).
  - When `crossOriginIsolated` and `SharedArrayBuffer`/`Atomics` are available, the same proxy can enable an optional SharedArrayBuffer ring fast path negotiated by `usb.ringAttach` (`apps/web/src/usb/usb_proxy_ring.ts`):
...
> `USBDevice` structured-clone / transferability support is **browser-dependent** and must be treated as a runtime capability. Probe this at runtime via the production WebUSB smoke-test panel (`apps/web/src/usb/webusb_panel.ts`) or the dedicated diagnostics page (`/webusb_diagnostics.html`).
>
...
See also: [`usb-and-input.md`](usb-and-input.md).
...
- `server.headers` → `pnpm run dev`
- `preview.headers` → `pnpm run preview` / `vite preview` (serving `dist/`)

See:

- `vite.harness.config.ts` for the repo-root Vite app (used by CI/Playwright)
- `apps/web/vite.config.ts` for the legacy/experimental `apps/web/` Vite app
...
- [Disk Image Streaming (HTTP Range + Auth + COOP/COEP)](storage.md)
- [Disk Image Streaming Service (Runbook)](storage.md)
- [05 - Storage Subsystem: Remote HTTP server requirements](storage.md#remote-http-server-requirements-rangecorsno-transform)
...
// Aero splits guest RAM from control/IPC buffers to avoid relying on >4GiB offsets (the shared memory layout decision).
const GUEST_RAM_MIB = 1024; // 512 / 1024 / 2048 / 3072 (best-effort)

function initializeMemory() {
    // If this fails, fall back to the single-threaded build (the WebAssembly build variants decision).
    ...
    // The runtime `guestToLinear` helper (implemented via `apps/web/src/arch/guest_ram_translate.ts`)
...
A small proof-of-concept app and its Playwright tests live in this repo (`apps/web/`, `tests/`) and validate the above in real engines.
...
See also:
- `tests/helpers/csp_server.mjs` for header templates
- Host-side capability detection: `src/platform/features.ts` (`jit_dynamic_wasm`)
- PoC app: `apps/web/public/wasm-jit-csp/`
...
`aero_storage::VirtualDisk`; see [`19-indexeddb-storage-story.md`](storage.md)
and [`20-storage-trait-consolidation.md`](storage.md).
...
Concrete layout used by `apps/web/gpu-cache/persistent_cache.ts`:
...
In `apps/web/gpu-cache/persistent_cache.ts`, this is exposed as:
...
```javascript
// Main thread coordinator (simplified; see `apps/web/src/runtime/coordinator.ts`)
...
class WorkerCoordinator {
    ...
    async initSharedMemory() {
        ...
        workers.forEach(([role, worker]) => {
            ...
            // attachments (see `apps/web/src/runtime/protocol.ts`).
            worker.postMessage({
                kind: 'init',
                role,
                ...
                // See `apps/web/src/arch/guest_phys.ts`.
...
>
> - `vmRuntime=legacy`: `apps/web/src/workers/cpu.worker.ts` (drives `WasmVm` / `WasmTieredVm`)
> - `vmRuntime=machine`: `apps/web/src/workers/machine_cpu.worker.ts` (drives the canonical `Machine`)
>
> Both use helpers in `apps/web/src/runtime/`.
>
> The real runtime may use `WasmTieredVm` (tiered/Tier-1 JIT) in addition to `WasmVm` (Tier-0). For the canonical
> mapping of `aero-wasm` exports to runtime paths, see [`../state/repo-state-and-structure.md`](../state/repo-state-and-structure.md) and
> [Canonical machine stack](../decisions/0014-canonical-machine-stack.md).
...
// `WasmApi` exported by `crates/aero-wasm` (see `apps/web/src/runtime/wasm_loader.ts`).
let wasm = null;
...
// (see `apps/web/src/workers/machine_cpu.worker.ts`).
let vm = null;
...
> `apps/web/src/audio/audio_worklet_ring.ts`, `apps/web/src/platform/audio_worklet_ring_layout.js`, `apps/web/src/platform/audio.ts`, and `apps/web/src/platform/audio-worklet-processor.js`.
>
> Note: In production builds, `audio-worklet-processor.js` is emitted as a standalone asset and imports
> `./audio_worklet_ring_layout.js` at runtime; see `apps/web/vite.config.ts` and `vite.harness.config.ts` for the explicit
...
> `CreateAudioOutputOptions.underrunMessageIntervalMs` in `apps/web/src/platform/audio.ts`.

#### Processor Registration

> Note: The code below is illustrative. The canonical implementation lives in:
>
> - `apps/web/src/platform/audio.ts` (ring buffer layout + producer)
> - `apps/web/src/audio/audio_worklet_ring.ts` + `apps/web/src/platform/audio_worklet_ring_layout.js` (ring buffer layout/constants + helper math)
...
> In Aero, prefer using the canonical helper `createAudioOutput` (`apps/web/src/platform/audio.ts`) instead of wiring Web Audio
> directly. It handles:
>
...
> - optional startup/latency-smoothing policies like `startupPrefillFrames` and `discardOnResume` (see `audio.md`)
...
async function setupAudio() {
    ...
    // See: `apps/web/src/platform/audio.ts` (`getMetrics`, `startAudioPerfSampling`).
...
All TCP egress uses the **Aero Gateway** backend (see `services/gateway`):
...
const compat = {
    async checkSupport() {
        ...
        if (!crossOriginIsolated || typeof SharedArrayBuffer === 'undefined') {
            issues.push('SharedArrayBuffer/threads not available - check COOP/COEP headers (see wiki: cross-origin isolation)');
        }
    ...
    async getStorageBackend() {
        ...
        // See wiki: the IndexedDB storage story, and storage trait consolidation.
...
- See [Testing Strategy](testing.md) for browser testing
- See [Performance Optimization](performance.md) for API-specific optimizations
