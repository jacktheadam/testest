...
param(
  ...
  # (via drivers/build/lib/New-IsoFile.ps1; deterministic via the Rust ISO writer; requires cargo unless -LegacyIso)
...
function Write-SyntheticInfUtf16LeNoBom {
  ...
  # drivers/build/package-guest-tools.ps1's contract auto-patching (HWID matching + AddService scanning)
...
function Assert-GuestToolsDevicesCmdServices {
  ...
  # `make-guest-tools-from-virtio-win.ps1` to pass `protocol-vectors/windows-device-contract-virtio-win.json`
  ...
  $contractPath = Join-Path $repoRoot "protocol-vectors\windows-device-contract-virtio-win.json"
...
# Validate `drivers/build/package-guest-tools.ps1` can auto-align Windows device contract driver_service_name
...
$contractTemplate = Join-Path $repoRoot "protocol-vectors\\windows-device-contract-virtio-win.json"
...
# Regression test: drivers/build/package-guest-tools.ps1 patches the Windows device contract by
...
$contractTemplatePath = Join-Path $repoRoot "protocol-vectors\\windows-device-contract.json"
...
$ciGuestToolsArgs = @(
  ...
  "-SigningPolicy", "none",
  "-SpecPath", $GuestToolsSpecPath,
  "-WindowsDeviceContractPath", "protocol-vectors/windows-device-contract-virtio-win.json",
