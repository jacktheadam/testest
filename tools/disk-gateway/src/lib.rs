...
use axum::http::header::{
    ...
    ETAG, IF_MODIFIED_SINCE, IF_NONE_MATCH, IF_RANGE, LAST_MODIFIED, ORIGIN, RANGE, VARY,
};
...
mod config_env_tests {
    ...
    fn from_env_rejects_non_utf8_bind() {
        ...
        let prev_bind = set_var_scoped_os(
            "DISK_GATEWAY_BIND",
        ...
        let err =
            Config::from_env().expect_err("expected Config::from_env to reject non-utf8 bind");
        match err {
            ConfigError::InvalidEnv(var) => assert_eq!(var, "DISK_GATEWAY_BIND"),
            other => panic!("expected InvalidEnv, got {other:?}"),
        }
...
const MAX_JWT_LEN: usize =
    MAX_JWT_HEADER_B64URL_LEN + 1 + MAX_JWT_PAYLOAD_B64URL_LEN + 1 + HMAC_SHA256_SIG_B64URL_LEN;
...
async fn serve_multi_range(
    ...
    // The stream outlives this call, so it needs its own copy rather than the borrow.
...
mod tests {
    ...
    async fn get_if_modified_since_returns_304() {
        ...
        assert_eq!(resp.status(), StatusCode::NOT_MODIFIED);
        assert_eq!(
            resp.headers().get(LAST_MODIFIED).unwrap().to_str().unwrap(),
            http_date
        );
    ...
    async fn range_supports_offsets_beyond_4gib_and_suffix_ranges() {
        ...
        assert_eq!(
            ...
            format!(
                "bytes {}-{}/{}",
                LARGE_HIGH_OFFSET, high_end, LARGE_FILE_SIZE
            )
        );
    ...
    fn parse_range_header_tolerates_whitespace() {
        ...
        assert_eq!(resolved, vec![ResolvedByteRange { start: 1, end: 3 }]);
    }
    ...
    fn apply_cors_headers_preserves_vary_star() {
        ...
        apply_cors_headers(&cfg, Some(&origin), &mut resp, true, "GET, HEAD, OPTIONS");
