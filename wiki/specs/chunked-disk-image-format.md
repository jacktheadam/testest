# Chunked disk image format

> The container that makes multi-gigabyte images streamable over plain HTTP
> without ranged requests: manifest layout, chunk addressing, integrity, and
> the client fetch strategy.
...
2. **CDN/object-store limitations:** many CDNs do not cache `206 Partial Content` responses well, vary cache keys in surprising ways, impose size/offset limits, or require special configuration to forward `Range` at all.
   - See: [17 - HTTP Range + CDN Behavior](../areas/storage.md) (CloudFront/Cloudflare behavior and size limits).
...
This format is designed to back the `StreamingDisk` concept described in [05 - Storage Subsystem](../areas/storage.md#image-download-and-streaming).

For how disk images are uploaded/imported and kept private in a hosted service (ownership/sharing/writeback strategies), see: [Disk Image Lifecycle and Access Control](../areas/storage.md).

For auth/CORS/COOP/COEP guidance that also applies to chunk manifests and chunk objects (even though this format avoids `Range`), see: [Disk Image Streaming (HTTP Range + Auth + COOP/COEP)](../areas/storage.md).
...
- This format does **not** support remote writes. Writes still go to a local overlay
  ...
  [`19-indexeddb-storage-story.md`](../areas/storage.md) and the canonical trait mapping
  in [`20-storage-trait-consolidation.md`](../areas/storage.md).
...
## Upload / Publish Pipeline (Server-side)
...
- `apps/web/src/storage/remote_chunked_disk.ts` (`RemoteChunkedDisk`)
...
In the `StreamingDisk` example in [05 - Storage Subsystem](../areas/storage.md#image-download-and-streaming), the remote path currently uses:
