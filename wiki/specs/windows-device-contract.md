...
**[`windows-device-contract.json`](../../protocol-vectors/windows-device-contract.json)**.

Editor note: the JSON manifests include a `$schema` reference to
[`windows-device-contract.schema.json`](../../protocol-vectors/windows-device-contract.schema.json) for improved
...
- [`windows-device-contract-virtio-win.json`](../../protocol-vectors/windows-device-contract-virtio-win.json)
...
- `drivers/build/package-guest-tools.ps1 -WindowsDeviceContractPath protocol-vectors/windows-device-contract-virtio-win.json`
...
3. Any breaking change requires updating:
   - `windows7-virtio-driver-contract.md` (virtio devices)
   - `windows-device-contract.md`
...
- When `MachineConfig::enable_aerogpu=true`, the machine exposes the AeroGPU PCI identity at
  ...
  `../areas/graphics.md`). Note: the in-tree Win7 AeroGPU driver treats the adapter as
...
- The Win7 KMD still has a compatibility path for the deprecated legacy bring-up AeroGPU ABI (the legacy `"ARGP"` device model; see `aerogpu-device-abi.md` for the exact PCI identity).
...
> See also: `aerogpu-device-abi.md` (context on why two IDs exist, and which emulator device models implement each ABI).
>
...
> Treat `windows7-virtio-driver-contract.md` as authoritative if this document ever disagrees.
