...
import { socketWritableLengthOrOverflow } from "../../../packages/transport-safety/src/socket_writable_length.cjs";
import { tryGetStringProp } from "../../../packages/transport-safety/src/safe_props.cjs";
import { unrefBestEffort } from "../../../packages/transport-safety/src/unref_safe.cjs";
...
} from "../../../packages/transport-safety/src/socket_safe.cjs";
import { wsCloseSafe, wsIsOpenSafe } from "./wsClose";
import { formatOneLineUtf8 } from "../../../packages/transport-safety/src/text.cjs";
  ...
  const handleOpen = (frame: TcpMuxFrame) => {
    ...
    void (async () => {
      ...
      tcpSocket.on("data", (chunk) => {
        ...
        metrics.addBytesOut("tcp_mux", frame.length);
        sendMuxFrame(TcpMuxMsgType.DATA, current.id, frame);
      });
