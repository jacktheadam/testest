...
export function allocateHarnessSharedMemorySegments(opts: {
  ...
  if (bytesToPages(requestedGuestBytes) <= 0) {
    throw new Error(`guestRamBytes must be > 0 (got ${requestedGuestBytes})`);
  }

  // Use the same layout the coordinator uses. The wasm module is linked with `--import-memory`
  // and `--stack-first`, so the bottom of linear memory belongs to its stack and static data and
  // guest RAM starts above it at `guest_base`. A harness that allocates only `guestRamBytes` and
  // declares `guest_base = 0` hands the module a memory smaller than its own declared minimum:
  // the import fails to satisfy, the glue quietly falls back to allocating its own memory, and
  // the worker ends up reading a buffer the module never writes to.
  const layout = computeGuestRamLayout(requestedGuestBytes);
  const pages = layout.wasm_pages;
  const guestRamBytes = layout.guest_size;
