...
```powershell
pwsh -File drivers/build/install-wdk.ps1
pwsh -File drivers/build/build-drivers.ps1 -ToolchainJson out/toolchain.json
# Required when drivers/aerogpu/ci-package.json declares dbgctl via `requiredBuildOutputFiles`.
pwsh -File drivers/build/build-aerogpu-dbgctl.ps1 -ToolchainJson out/toolchain.json
pwsh -File drivers/build/make-catalogs.ps1 -ToolchainJson out/toolchain.json
pwsh -File drivers/build/sign-drivers.ps1 -ToolchainJson out/toolchain.json
pwsh -File drivers/build/package-drivers.ps1 -MakeFatImage
pwsh -File drivers/build/package-guest-tools.ps1
```

## `drivers/build/build-drivers.ps1`

Builds driver projects under `drivers/` for the requested platforms/configuration using `msbuild.exe`.

## `drivers/build/build-aerogpu-dbgctl.ps1`
...
- `drivers/build/build-aerogpu-dbgctl.ps1` imports an x86 Visual Studio developer environment (VsDevCmd/vcvarsall) and
  validates the produced binary is `IMAGE_FILE_MACHINE_I386 (0x014c)`, failing fast if it is x64 (0x8664) or unknown.

In CI, this script is run after `drivers/build/build-drivers.ps1` so it can also copy the built tool into
...
`requiredBuildOutputFiles` — `drivers/build/make-catalogs.ps1` will fail packaging if the tool was not staged into
the per-arch build output directories.

### CI packaging gate (`ci-package.json`)

For CI determinism (and to avoid accidentally shipping dev/test drivers), `drivers/build/build-drivers.ps1` only
builds drivers that are explicitly opted into CI packaging by placing `ci-package.json` at the driver
root.

See:

  - `drivers/build/driver-package.schema.json`
...
The modern toolchain installed by `drivers/build/install-wdk.ps1` does not provide `build.exe`, so CI **skips**
...
```powershell
pwsh -File drivers/build/build-drivers.ps1 -ToolchainJson out/toolchain.json -IncludeMakefileProjects
```

## `drivers/build/stamp-infs.ps1`
...
## `drivers/build/make-catalogs.ps1`

Runs `drivers/build/stamp-infs.ps1` **before** calling `Inf2Cat.exe`, because catalog hashes include the INF contents.

Note: `Inf2Cat` catalog membership is toolchain-dependent (see the `drivers/build/validate-toolchain.ps1` smoke test note below). If you ship auxiliary tools or other extra files inside the staged driver package directory (for example under `tools/`), stage them **before** this step so the package contents used for catalog generation are final.
...
After staging each driver+arch package directory, `drivers/build/make-catalogs.ps1` performs a sanitation pass over the
...
`drivers/build/make-catalogs.ps1` in a follow-up (we prefer explicit allowlist additions over copying all build outputs
verbatim).
...
- To include `WdfCoInstaller*.dll`, a driver must declare `wdfCoInstaller` in `drivers/<driver>/ci-package.json` and `drivers/build/make-catalogs.ps1` must be run with `-IncludeWdfCoInstaller` (or `-IncludeWdkRedist WdfCoInstaller`).
...
CI’s Win7 toolchain smoke test (`drivers/build/validate-toolchain.ps1`) includes a minimal experiment that adds an unreferenced file under `tools/` and checks whether its name appears in the generated `.cat` (via `certutil -dump`, with a raw-byte fallback). The logs include a stable summary line:
...
Practical rule: treat staged package directories as immutable after `drivers/build/make-catalogs.ps1` runs. For packaged helper tools (for example `aerogpu_dbgctl.exe` staged as `tools/win7_dbgctl/bin/aerogpu_dbgctl.exe`), build/copy them into the staging directory *before* catalog generation.
...
## `drivers/build/package-drivers.ps1`
...
`drivers/build/package-drivers.ps1` deletes a staging directory at `$OutDir/_staging`. To prevent accidental
...
`drivers/build/package-drivers.ps1` builds the bundle ISO using the deterministic Rust
...
- ISO creation requires `cargo` by default (deterministic, cross-platform).
- Use `-NoIso` to skip ISO creation.
- Use `-LegacyIso` to force the legacy Windows IMAPI2 path (**not deterministic**, Windows only).

Notes:

- `drivers/build/package-drivers.ps1 -DeterminismSelfTest` checks that repeated runs produce identical `*-x86.zip`, `*-x64.zip`, and `*-bundle.zip` artifacts (and their `*.manifest.json` files), and identical `*.iso` artifacts (and its manifest) when ISO creation is enabled.
- The legacy helper `drivers/build/lib/New-IsoFile.ps1` also requires `cargo`/`aero_iso` by default, and uses IMAPI2 only when `-LegacyIso` is provided.
...
`drivers/build/package-drivers.ps1` stages drivers into `drivers/<driverName>/<arch>/...` inside the output zip/iso.
...
```powershell
pwsh -File drivers/build/package-drivers.ps1 -SelfTest
```
...
packages staged under `out/packages/` (the output of `drivers/build/make-catalogs.ps1` + `drivers/build/sign-drivers.ps1`).

You can also point `-InputRoot` at an extracted `*-bundle.zip` (or the `.zip` file itself)
produced by `drivers/build/package-drivers.ps1`.
...
`drivers/build/package-guest-tools.ps1` normalizes driver directory names (lowercase + some legacy aliases like
...
This script is intended to be run after `drivers/build/package-drivers.ps1` in CI so that the release
contains both:
...
contract JSON during packaging (default: `protocol-vectors/windows-device-contract.json`). Use
`drivers/build/package-guest-tools.ps1 -WindowsDeviceContractPath` to override the contract when packaging
...
`drivers/build/package-guest-tools.ps1` can optionally stage additional guest-side helper binaries/scripts
under `tools/` in the packaged ISO/zip without requiring them to be checked into `guest-tools/`:

```powershell
pwsh -File drivers/build/package-guest-tools.ps1 -ExtraToolsDir out/guest-tools-extra
```
...
```powershell
pwsh -File drivers/build/package-guest-tools.ps1 -ExtraToolsDir out/guest-tools-extra -ExtraToolsDirMode replace
```
...
`drivers/build/package-guest-tools.ps1` uses `-SpecPath` to control which driver directories are required/allowed
...
## `drivers/build/make-fat-image.ps1`
