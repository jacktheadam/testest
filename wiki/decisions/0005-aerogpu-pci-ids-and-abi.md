# AeroGPU PCI IDs and ABI

**Decision (0005).** `A3A0:0001` is AeroGPU's canonical PCI identity. The bring-up identity is deprecated on a stated path rather than removed abruptly, because guest drivers bind to it.
...
- `wiki/areas/windows-drivers.md` + `protocol-vectors/windows-device-contract.json` document the canonical Windows-facing AeroGPU binding contract (**A3A0**) and are checked in CI; older experiments (a retired cmd/completion-ring prototype ABI) used different PCI identities/ABIs and have been retired (see `wiki/history/retirements.md`).
...
- `MachineConfig::enable_aerogpu=true` (and `enable_vga=false`): boot display is provided via
  ...
  [`wiki/areas/graphics.md`](../areas/graphics.md)).
...
- [`wiki/areas/windows-drivers.md`](../areas/windows-drivers.md)
...
- **Docs, INFs, and device models must stay in sync.** Any change to AeroGPU’s PCI identity or ABI must update all consumers (protocol headers, emulator/device model, INFs, and `wiki/areas/windows-drivers.md` + `protocol-vectors/windows-device-contract.json`) in the same change set.
...
- **CI drift checks are required.** CI must detect mismatches between:
  ...
  - `protocol-vectors/windows-device-contract.json` (canonical Windows contract),
