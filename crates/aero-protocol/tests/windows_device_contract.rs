...
fn windows_device_contract_aerogpu_matches_protocol_constants() {
    ...
    assert_eq!(AEROGPU_PCI_BAR1_INDEX, 1);
    assert_eq!(AEROGPU_PCI_BAR1_SIZE_BYTES, 64u32 * 1024u32 * 1024u32);
    // Canonical BAR1 layout reserves the first 256KiB for legacy VGA planar memory; the packed-pixel
    // VBE LFB begins immediately after that region.
    assert_eq!(AEROGPU_PCI_BAR1_VBE_LFB_OFFSET_BYTES, 0x40_000);

    assert_eq!(AEROGPU_PCI_SUBSYSTEM_VENDOR_ID, AEROGPU_PCI_VENDOR_ID);
    assert_eq!(AEROGPU_PCI_SUBSYSTEM_ID, 0x0001);

    let root = repo_root();
    let contract_path = root.join("protocol-vectors/windows-device-contract.json");
    ...
    let contract_md_path = root.join("wiki/specs/windows-device-contract.md");
...
#[test]
fn windows_device_contract_is_located_in_protocol_vectors() {
    let root = repo_root();
    assert!(
        Path::new(&root)
            .join("protocol-vectors/windows-device-contract.json")
            .is_file(),
        "protocol-vectors/windows-device-contract.json must exist relative to repo root"
    );
}
...
fn no_aerogpu_1aed_tokens_outside_quarantined_legacy_locations() {
    ...
    //   - wiki/areas/graphics.md (mapping doc / source-of-truth context)
    ...
    let allowed_mapping_doc = b"wiki/areas/graphics.md";
