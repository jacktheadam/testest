...
fn input_reports_missing_node_modules_with_node_dir_rust_only_hint() {
    ...
    Command::new(env!("CARGO_BIN_EXE_xtask"))
        ...
        .stderr(predicate::str::contains(
            "cargo xtask input --node-dir apps/web --rust-only",
        ));
...
#[test]
#[cfg(unix)]
fn input_reports_missing_package_manager_with_rust_only_hint() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
    ...
    Command::new(env!("CARGO_BIN_EXE_xtask"))
        ...
        .failure()
        .code(1)
        .stderr(predicate::str::contains("missing required command: pnpm"))
        .stderr(predicate::str::contains("cargo xtask input --rust-only"));
}
...
#[test]
#[cfg(unix)]
fn input_reports_missing_package_manager_with_node_dir_rust_only_hint() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
...
fn input_invokes_expected_web_unit_test_subset() {
    ...
    let pnpm_unit = invocations
        .iter()
        .find(|argv| {
            argv.first().map(|s| s.as_str()) == Some("pnpm")
                && argv.contains(&"test:unit".to_string())
        })
        .expect("expected a pnpm test:unit invocation");
    ...
    let idx_scope = idx(pnpm_unit, "-C");
    let idx_web = idx(pnpm_unit, "apps/web");
    let idx_run = idx(pnpm_unit, "run");
    let idx_test_unit = idx(pnpm_unit, "test:unit");
    let idx_sep = idx(pnpm_unit, "--");

    assert!(
        idx_scope < idx_web
            && idx_web < idx_run
            && idx_run < idx_test_unit
            && idx_test_unit < idx_sep,
        "unexpected npm argument ordering: argv={pnpm_unit:?}"
    );

    for (i, expected) in EXPECTED_WEB_UNIT_TEST_PATHS.iter().enumerate() {
        let actual = pnpm_unit
            .get(idx_sep + 1 + i)
            .unwrap_or_else(|| panic!("missing `{expected}` in npm argv={pnpm_unit:?}"));
        assert_eq!(
            actual, expected,
            "unexpected npm test subset ordering; argv={pnpm_unit:?}"
        );
    }
}
...
fn input_invokes_expected_web_unit_test_subset_when_node_dir_is_web() {
    ...
    assert!(
        !pnpm_unit.iter().any(|arg| arg == "-C"),
        "expected AERO_NODE_DIR=web to avoid `npm -w web`; argv={pnpm_unit:?}"
    );
    assert!(
        !pnpm_unit.iter().any(|arg| arg == "apps/web"),
        "expected AERO_NODE_DIR=web to avoid a literal `web` workspace arg; argv={pnpm_unit:?}"
    );
...
fn input_invokes_expected_web_unit_test_subset_when_aero_web_dir_is_web() {
    ...
    assert!(
        !pnpm_unit.iter().any(|arg| arg == "-C"),
        "expected AERO_WEB_DIR=web to avoid `npm -w web`; argv={pnpm_unit:?}"
    );
    assert!(
        !pnpm_unit.iter().any(|arg| arg == "apps/web"),
        "expected AERO_WEB_DIR=web to avoid a literal `web` workspace arg; argv={pnpm_unit:?}"
    );
...
fn input_invokes_expected_web_unit_test_subset_when_web_dir_is_web() {
    ...
    assert!(
        !pnpm_unit.iter().any(|arg| arg == "-C"),
        "expected WEB_DIR=web to avoid `npm -w web`; argv={pnpm_unit:?}"
    );
    assert!(
        !pnpm_unit.iter().any(|arg| arg == "apps/web"),
        "expected WEB_DIR=web to avoid a literal `web` workspace arg; argv={pnpm_unit:?}"
    );
...
fn input_invokes_expected_web_unit_test_subset_when_node_dir_flag_is_web() {
    ...
    assert!(
        !pnpm_unit.iter().any(|arg| arg == "-C"),
        "expected --node-dir web to avoid `npm -w web`; argv={pnpm_unit:?}"
    );
    assert!(
        !pnpm_unit.iter().any(|arg| arg == "apps/web"),
        "expected --node-dir web to avoid a literal `web` workspace arg; argv={pnpm_unit:?}"
    );
...
fn input_invokes_expected_web_unit_test_subset_when_web_dir_flag_is_web() {
    ...
    assert!(
        !pnpm_unit.iter().any(|arg| arg == "-C"),
        "expected --web-dir web to avoid `npm -w web`; argv={pnpm_unit:?}"
    );
    assert!(
        !pnpm_unit.iter().any(|arg| arg == "apps/web"),
        "expected --web-dir web to avoid a literal `web` workspace arg; argv={pnpm_unit:?}"
    );
...
fn hide_node_modules(repo_root: &Path) -> std::io::Result<NodeModulesHideGuard> {
    ...
    let web_node_modules = repo_root.join("apps/web/node_modules");
