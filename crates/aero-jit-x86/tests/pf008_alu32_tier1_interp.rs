...
fn pf008_alu32_checksum() {
    // From the CPU and JIT area page (PF-008), `alu32` payload.
    //
    // BITS 32
    // mov eax, 0x9ABCDEF0
    // mov edx, 0x7F4A7C15
...
fn pf008_mem_seq32_checksum() {
    // From the CPU and JIT area page (PF-008), `mem_seq32` payload.
    let code = [
...
fn pf008_call_ret32_checksum() {
    // From the CPU and JIT area page (PF-008), `call_ret32` payload.
    let code = [
...
fn pf008_branch_pred32_checksum() {
    // From the CPU and JIT area page (PF-008), `branch_pred32` payload.
    let code = [
...
fn pf008_branch_unpred32_checksum() {
    // From the CPU and JIT area page (PF-008), `branch_unpred32` payload.
    let code = [
...
fn pf008_mem_stride32_checksum() {
    // From the CPU and JIT area page (PF-008), `mem_stride32` payload.
    let code = [
        0xb8, 0xef, 0xcd, 0xab, 0x89, 0x31, 0xf6, 0x8b, 0x14, 0x37, 0x01, 0xd0, 0x31, 0xc2, 0x89,
