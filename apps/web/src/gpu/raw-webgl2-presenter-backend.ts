...
/**
 * Drop any WebGL error left over from earlier work, so the next {@link assertWebGlOk} reports only
 * what happened in between.
 *
 * `getError()` returns the oldest error recorded since it was last called and clears one flag per
 * call. Without this, an unchecked failure anywhere upstream is reported against whichever
 * operation happens to assert next — during context-loss recovery that surfaced as a bogus
 * `texImage2D: INVALID_OPERATION` on a plain 64x64 upload, and aborted the recovery.
 */
function clearWebGlErrors(gl: WebGL2RenderingContext): void {
  // Bounded: a context that keeps reporting errors (e.g. a lost one) must not spin here.
...
export class RawWebGl2Presenter implements Presenter {
  ...
  /**
   * `WEBGL_lose_context`, captured while the context is still alive.
   *
   * `getExtension()` returns null once a context is lost, so a handle fetched on demand is only
   * ever good for losing the context — never for restoring one. `restoreContext()` has to be
   * called on an object obtained beforehand.
   */
  private loseContextExt: { loseContext?: () => void; restoreContext?: () => void } | null = null;
  ...
  public init(canvas: OffscreenCanvas, width: number, height: number, dpr: number, opts?: PresenterInitOptions): void {
    ...
    this.gl = gl;
    this.loseContextExt = gl.getExtension('WEBGL_lose_context') as unknown as {
      loseContext?: () => void;
      restoreContext?: () => void;
    } | null;
  ...
  public debugRestoreContext(): boolean {
    ...
    // `webglcontextrestored` is not guaranteed on an OffscreenCanvas in a worker, and the
    // context can come back without it. Poll `isContextLost()` as well, and run the same
    // restoration path the event handler would; `handleContextRestored` is idempotent.
    let attempts = 0;
    const poll = () => {
      if (!this.isContextLost) return;
      const gl = this.gl;
  ...
  /**
   * Rebuild GPU resources after the context came back, and tell the owner.
   *
   * Reached either from `webglcontextrestored` or from the poll in
   * {@link debugRestoreContext}; whichever gets there first wins, and the second is a no-op.
   */
