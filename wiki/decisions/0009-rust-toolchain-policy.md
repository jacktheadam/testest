...
**Decision (0009).** A pinned stable Rust toolchain everywhere, with a pinned nightly only where threaded WebAssembly requires one.
...
- All nightly-dependent entrypoints must use this pinned nightly toolchain:
  - `apps/web/scripts/build_wasm.mjs` (threaded variant)
  - `just setup` (installs the pinned nightly + `rust-src`)
  - CI (threaded WASM smoke build)
  - CI fuzz smoke workflow (cargo-fuzz)
- There is no CI here; when a nightly toolchain is needed, take it from
  `scripts/toolchains.json` rather than a workflow input. (Historically: `./.github/actions/setup-rust`, which resolves
