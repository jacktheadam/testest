...
export interface PresenterInitOptions {
  ...
  /**
   * Called after a lost context has been restored and the backend's resources rebuilt.
   *
   * Only `onError` fires on the failure path, so without this a successful recovery is silent and
   * nothing downstream can tell that the presenter is usable again.
   */
