...
> The networking backend's externally visible contract: endpoints, session
> handling, the DNS-over-HTTPS surface, error semantics, and the limits the
> gateway enforces.
...
- An **L2 tunnel** exposed as a WebSocket endpoint (`/l2`; legacy alias: `/eth`) for Option C networking (see `l2-tunnel-protocol.md`)
...
HTTP request/response schemas are specified in [`services/gateway/openapi.yaml`](../../services/gateway/openapi.yaml). The `/tcp` WebSocket upgrade is documented in this document (OpenAPI intentionally does not model WebSockets).
...
## Session model (`POST /session`, cookies)
...
- Sets cookie:
  ...
  - Attributes (recommended):
    ...
    - `Path=/`
- Body: JSON with session metadata and limits (see `services/gateway/openapi.yaml`).
...
- `endpoints.l2` is routed by the **edge reverse proxy** to `aero-l2-proxy` (the Rust L2 tunnel proxy). It is not served by the `services/gateway` Node.js process.
...
- `limits.l2` describes protocol-level payload limits for the L2 tunnel (`FRAME` vs control messages).
  ...
  - Recommended defaults are `2048` bytes for `FRAME` payloads and `256` bytes for control payloads (see [`l2-tunnel-protocol.md`](l2-tunnel-protocol.md)).
...
See also: [`auth-tokens.md`](auth-tokens.md) (formats + strict verification contract).
...
## `/tcp` WebSocket TCP proxy protocol
...
- Gateway codec + handler:
  ...
  - `services/gateway/src/routes/tcpMux.ts`
- Browser client (TypeScript):
  - `apps/web/src/net/tcpMuxProxy.ts`
- Dev relays (Node):
  - `services/net-proxy/` (local development relay; `/tcp-mux` endpoint + DoH `/dns-query` + `/dns-json`)
  - `tools/net-proxy-server/` (standalone dev relay; `/tcp-mux` with `?token=` auth)

## `/dns-query` DNS-over-HTTPS (DoH)
...
> Local development note: the repo also includes [`services/net-proxy/`](../../services/net-proxy/) which exposes `/dns-query` and
> `/dns-json` so you can run local networking without the full gateway. `net-proxy`’s DoH endpoints are intentionally
> lightweight (and are unauthenticated / not policy-filtered); see [`services/net-proxy/README.md`](../decisions/README.md).
...
## Security model (gateway responsibilities)
...
## Recommended browser-side integration patterns
