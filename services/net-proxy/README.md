...
This lets you run local networking (TCP + UDP + DNS) without standing up the full `services/gateway` service.
...
If you're using the browser networking clients in `apps/web/src/net`:
...
// In this repo they live under `apps/web/src/net` (e.g. `src/main.ts` imports from `../web/src/...`).
...
>
> ```bash
> AERO_PROXY_DOH_CORS_ALLOW_ORIGINS='http://localhost:5173' npm -w services/net-proxy run dev
> ```
>
...
See also: [`wiki/areas/networking.md`](wiki/areas/networking.md) (where DoH fits into the overall networking stack).
...
| `AERO_PROXY_DOH_CORS_ALLOW_ORIGINS` | (empty) | Comma-separated CORS origin allowlist for `/dns-query` and `/dns-json` (e.g. `http://localhost:5173`). Use `*` only in trusted local dev. You can also explicitly allow opaque `Origin: null` contexts (e.g. `file://...`) by including `null`. Includes basic Private Network Access (PNA) preflight support (`Access-Control-Allow-Private-Network`), preflight caching (`Access-Control-Max-Age`), and exposes `Content-Length` (`Access-Control-Expose-Headers`). |
