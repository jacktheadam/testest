...
fn docs_do_not_reference_retired_a0e0_abi() {
    // Documentation now lives in the wiki/ tree (the old docs/ tree is purged).
    let docs_root = repo_root().join("wiki");
    assert!(
        docs_root.is_dir(),
        "expected wiki directory at {}",
        docs_root.display()
    );
    ...
    assert!(
        offenders.is_empty(),
        "wiki docs must not reference the retired A0E0 AeroGPU ABI; offending files:\n{}",
        offenders
            .iter()
