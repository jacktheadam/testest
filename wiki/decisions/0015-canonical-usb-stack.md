...
**Decision (0015).** `crates/aero-usb` together with `apps/web/src/usb/*` is the browser USB stack.
...
- **Browser/WASM path (active):**
  ...
  - Host integration (WebHID/WebUSB broker/executor, worker proxying): `apps/web/`
...
### Canonical USB stack for the browser runtime
...
- **USB device models + host controllers (Rust, deterministic, wasm32-friendly):** `crates/aero-usb`
  ...
  - EHCI (USB 2.0) and xHCI (USB 3.x) models also live in `crates/aero-usb` and are being brought up
    ...
    - See controller docs: [`wiki/areas/usb-and-input.md`](../areas/usb-and-input.md) (EHCI/xHCI controller notes).
...
- **Host integration (TypeScript):** `apps/web/src/usb/*`
  - WebUSB executor/broker (main thread): `apps/web/src/usb/webusb_backend.ts`, `apps/web/src/usb/usb_broker.ts`
  - Worker-side runtime pump: `apps/web/src/usb/webusb_passthrough_runtime.ts`

Note: the canonical browser host entrypoint is the repo-root Vite app, but it imports shared runtime
modules from `apps/web/src/*`. The `apps/web/` directory’s own `apps/web/bringup.html` entrypoint is legacy, but the
USB host integration under `apps/web/src/usb/*` is the maintained implementation.

The browser runtime must **not** implement a parallel USB stack in `crates/emulator` or in
TypeScript.

### Canonical ownership of the WebUSB passthrough wire contract
...
- **Cross-language fixture:** `protocol-vectors/webusb_passthrough_wire.json`
- **TypeScript mirror types:** `apps/web/src/usb/usb_passthrough_types.ts`

Any change to the wire contract must update **all three** in a single change set and keep both the
Rust and TS tests passing.

### Status of the emulator USB module (`crates/emulator`, `emulator::io::usb`)
...
### Where UHCI lives long-term, and how it connects to `aero_machine::Machine`
  ...
  - main thread ↔ worker proxying (default: `postMessage` with transferred `ArrayBuffer`s).
    When `globalThis.crossOriginIsolated === true` and `SharedArrayBuffer`/`Atomics` are available:
      - WebHID passthrough uses SAB ring buffers to avoid per-report messaging overhead:
        ...
        - Implementation:
          - `apps/web/src/ipc/ring_buffer.ts` (`RingBuffer`)
          - `apps/web/src/hid/hid_input_report_ring.ts` (record codec + writer)
          - `apps/web/src/hid/hid_proxy_protocol.ts` (`hid.ring.init`, `hid.ringAttach`, `hid.ringDetach`)
          - `apps/web/src/hid/webhid_broker.ts` (runtime selection + producers)
          - `apps/web/src/workers/io_hid_input_ring.ts` (worker drain helper)
    - WebUSB passthrough supports a SAB ring-buffer fast path for host action/completion proxying,
      ...
      - Implementation:
        - `apps/web/src/usb/usb_proxy_ring.ts`
        - `apps/web/src/usb/usb_proxy_protocol.ts` (`usb.ringAttach`, `usb.ringAttachRequest`, `usb.ringDetach`)
        - `apps/web/src/usb/usb_broker.ts`
        - `apps/web/src/usb/webusb_passthrough_runtime.ts`
        - `apps/web/src/usb/usb_proxy_ring_dispatcher.ts` (completion-ring fan-out when multiple runtimes subscribe)
...
alive and call `reset()` on disconnect instead of recreating it. See `wiki/areas/usb-and-input.md`.
- Long-term, the UHCI controller should be integrated into the canonical VM wiring described by
  [Canonical machine stack](0014-canonical-machine-stack.md):
...
- New work has a single default place to land: `crates/aero-usb` (and `apps/web/src/usb/*` for host
  integration).
...
- `crates/aero-usb` device models can participate in VM snapshot/restore via the repo-standard
  ...
  the `DEVICES` section (see `wiki/areas/platform-and-firmware.md`).
...
1. **Docs**
   ...
   - See also: [`wiki/areas/platform-and-firmware.md`](../areas/platform-and-firmware.md) (broader `crates/emulator` → canonical stack plan + deletion targets).
...
3. **Consolidate TypeScript WebUSB host integration**
   - Treat `apps/web/src/usb/*` as the canonical WebUSB passthrough host integration for the
    ...
    - The repo previously included a **generic WebUSB demo RPC** stack (direct `navigator.usb` operations)
      ...
      a parallel USB stack or an alternate passthrough wire contract outside `apps/web/src/usb/*` + `crates/aero-usb`.
...
- Keep the USB passthrough and host controller behavior (UHCI/EHCI/xHCI) covered by `crates/aero-usb` tests:
  ...
  - EHCI tests (regs + root hub timers + async/periodic schedule walking + snapshot roundtrips):
    `crates/aero-usb/tests/ehci*.rs` (see [`wiki/areas/usb-and-input.md`](../areas/usb-and-input.md))
...
- Keep the wire contract fixture stable:
  - `protocol-vectors/webusb_passthrough_wire.json` must round-trip with Rust types.
