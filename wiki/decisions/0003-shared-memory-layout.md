...
**Decision (0003).** Use several shared buffers rather than one. A wasm32 linear memory caps at 4 GiB, and one buffer for everything would spend that budget on whichever subsystem grew first.
...
- VRAM base paddr: `VRAM_BASE_PADDR = PCI_MMIO_BASE = 0xE000_0000` (see
  [`apps/web/src/arch/guest_phys.ts`](../../apps/web/src/arch/guest_phys.ts)).
...
- TS layout mirror used by the coordinator: `apps/web/src/runtime/shared_layout.ts` (`computeGuestRamLayout`)
...
- Guest RAM backing in wasm linear memory is still a **single contiguous byte buffer** of length
  ...
  - JS/TS code must translate guest physical addresses via
    `apps/web/src/runtime/shared_layout.ts::{guestPaddrToRamOffset,guestToLinear}` (implemented via
...
- The WASM build reserves a tiny tail guard at the *end* of the runtime-reserved region so the web runtime can safely use a deterministic scratch word for JS↔WASM memory wiring probes (without overlapping real Rust heap allocations).
  - Current tail guard size: **144 bytes** (`HEAP_TAIL_GUARD_BYTES` in `crates/aero-wasm/src/runtime_alloc.rs`) — a 32-byte scanout state, a 48-byte cursor state, and a 64-byte probe window.
  ...
  - See `crates/aero-wasm/src/runtime_alloc.rs` (`HEAP_TAIL_GUARD_BYTES`) and `apps/web/src/runtime/wasm_memory_probe.ts`.
...
- Shared-memory segment allocation + layout computation: [`apps/web/src/runtime/shared_layout.ts`](../../apps/web/src/runtime/shared_layout.ts)
...
- WASM build flags (imported memory, max memory, stack placement): [`apps/web/scripts/build_wasm.mjs`](../../apps/web/scripts/build_wasm.mjs)
- IPC protocol (binary rings + atomics contracts): [`wiki/areas/platform-and-firmware.md`](../areas/platform-and-firmware.md)
