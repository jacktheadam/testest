export type PlatformFeatureReport = {
   ...
   * (HTTPS / localhost). We explicitly gate on `globalThis.isSecureContext` so
...
const WASM_SIMD_VALIDATION_BYTES = new Uint8Array([
  // From wiki/areas/web-host.md (WASM SIMD feature detection snippet).
...
export function explainMissingRequirements(
  report: PlatformFeatureReport = platformFeatures,
): string[] {
