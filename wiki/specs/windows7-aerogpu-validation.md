# Windows 7 AeroGPU validation

> The bring-up and stability checklist for the AeroGPU driver in a real
> Windows 7 guest: the ordered smoke tests, the timeout-detection-and-recovery
> strategy, the vblank expectations, the performance baseline to capture, and
> the debug playbook.
...
## Test environment assumptions (so results are comparable)
...
### Vblank and vsynced present pacing validation
...
## Bring-up checklist (smoke tests in strict order)
...
## TDR strategy (design + dev-only knobs)
...
## Vblank / vsync expectations (what Win7 + DWM want)
...
* Direct vblank interrupt/wait path (independent of DWM): `drivers/aerogpu/tests/win7/wait_vblank_pacing` (targets VidPn source 0; tune hang detection via `--wait-timeout-ms`)
  ...
  * Cross-process shared surface open (validates `DxgkDdiOpenAllocation` + ShareToken-backed shared-surface interop across runtimes):
    * Canonical `share_token` contract: `aerogpu-device-abi.md`
...
## Performance baseline (what to measure and how to capture it)
...
## Debug playbook (with `aerogpu_dbgctl`)
...
- Guest Tools ISO/zip:
  ...
  - Optional top-level tools payload (when present): `tools\\aerogpu_dbgctl.exe` (or `tools\\<arch>\\aerogpu_dbgctl.exe`)
- CI-staged packages (after `drivers/build/make-catalogs.ps1`):
