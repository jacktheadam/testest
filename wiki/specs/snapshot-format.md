...
> Saving and restoring a running machine: the container layout, per-device
> state encoding, the determinism rules every save-capable device must follow,
> and the treatment of host resources that cannot be restored bit-for-bit.
...
- `DISKS` entries are written in canonical order: `disk_id`.
  ...
  - Canonical Windows 7 storage topology mapping (see [`../areas/storage.md`](../areas/storage.md)):
...
- **ACPI PM determinism:** `PM_TMR` must be derived from deterministic virtual time for stable snapshot/restore. Avoid basing it on host wall-clock / `Instant` (which will make snapshots nondeterministic and can introduce time jumps on restore). A register read is a pure sample and must never advance a private PM-timer timebase; any poll-loop acceleration must advance CPU and shared platform time coherently.
...
cause the restored `PM_TMR` value/phase to differ across restores. Reading the register is a pure sample:
it must not mutate a PM-timer-private clock. A host polling optimization may move virtual time forward only
by advancing the CPU TSC and the shared platform clock together before the next guest read.
...
In the worker-based web runtime, device blobs are exchanged as `{ kind: string, bytes: Uint8Array }` (protocol types in `apps/web/src/runtime/snapshot_protocol.ts`). The `kind` field is a **stable string** that maps to a numeric `DeviceId` (mapping lives in `apps/web/src/workers/vm_snapshot_wasm.ts` and is mirrored in `crates/aero-wasm/src/vm_snapshot_device_kind.rs` (used by the WASM-side snapshot builder)).
...
- When multiple controllers are present (UHCI + optional EHCI/xHCI), encode their per-controller
  ...
  (`apps/web/src/workers/usb_snapshot_container.ts`):
...
- The outer web snapshot `kind` for `DeviceId::USB` is `usb` (legacy kind `usb.uhci` accepted; see
  `apps/web/src/workers/vm_snapshot_wasm.ts`).
...
Restore notes (see also [`../areas/audio.md`](../areas/audio.md#snapshotrestore-save-states)):
...
mapping documented in [`../areas/storage.md`](../areas/storage.md).
...
### Full snapshot (chunked + optional compression)
...
### Dirty-page diff snapshot (incremental)
...
OPFS access is via `navigator.storage.getDirectory()` and `FileSystemFileHandle` APIs (see `../areas/storage.md` for OPFS notes).
...
Networking-specific restore semantics (what is and is not bit-restorable) are documented in `../areas/networking.md`.
