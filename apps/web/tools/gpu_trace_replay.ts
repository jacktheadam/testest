...
(function () {
  ...
  function formatOneLineErrorText(err, maxBytes) {
  ...
  async function maybeInitAerogpuProtocol() {
    ...
    aerogpuProtocolInit = (async () => {
      try {
        const cmd = await import("/crates/aero-protocol/aerogpu/aerogpu_cmd.ts");
        const pci = await import("/crates/aero-protocol/aerogpu/aerogpu_pci.ts");
        if (!cmd) return;
...
`;
  ...
  async function saveToOpfs(path, bytesLike) {
    ...
    // No type annotation: the golden-image specs inject this file into the page verbatim, so it
    // has to parse as plain JavaScript. The initialiser gives TypeScript the type anyway.
    let writable = /** @type {FileSystemWritableFileStream | undefined} */ (undefined);
    ...
    if (truncateFallback) {
      try {
        // Reached through an index rather than a cast: this file is injected verbatim into the
        // page by the golden-image specs, so it has to parse as plain JavaScript. Two `as` casts
        // here were enough to make the whole tool unloadable and every trace-replay golden fail
        // on `window.AeroGpuTraceReplay` being undefined.
        const maybeTruncate = /** @type {unknown} */ (writable)["truncate"];
        if (typeof maybeTruncate === "function") {
          await maybeTruncate.call(writable, 0);
        }
      } catch {
        // ignore
      }
    ...
    function executeCmdStream(cmdStreamBytes, memAllocs) {
      ...
      if (AerogpuCmdStreamIter) {
        ...
        try {
          ...
          fail(formatOneLineErrorText(err, 512));
        }
