...
```bash
npm -w services/gateway run dev
```
...
```bash
docker build -f services/gateway/Dockerfile -t aero-gateway . \
...
- `GET ws(s)://<host>/tcp?...` TCP proxy upgrade endpoint (WebSocket; see `wiki/history/retirements.md` and `deploy/README.md`)
...
```bash
node services/gateway/examples/tcp-mux-client.js http://127.0.0.1:8080 example.com 80
```
...
```bash
services/gateway/scripts/generate-dev-cert.sh
```

It writes a self-signed `localhost` certificate to:

```
services/gateway/.certs/localhost.crt
services/gateway/.certs/localhost.key
```
