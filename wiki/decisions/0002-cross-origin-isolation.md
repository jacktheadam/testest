...
**Decision (0002).** Serve the application under COOP and COEP. Cross-origin isolation is what makes `SharedArrayBuffer` and WebAssembly threads available at all, so it is a precondition rather than a hardening measure.
...
Also provide a **non-threaded fallback build** for environments where cross-origin isolation is not possible (see the WebAssembly build variants decision).
...
see [`wiki/areas/web-host.md`](../areas/web-host.md).
