# Aero architecture overview

> The whole system in one page: what Aero is, the guest-to-pixel path, the
> process and worker topology, and where each subsystem's detail lives. Read
> this first, then follow the area pages for depth.

## What Aero is

Aero is a Windows 7 SP1 x64 emulator built to run an unmodified guest in a
browser tab. The emulator core is Rust compiled to WebAssembly, split across Web
Workers that share guest physical memory through `SharedArrayBuffer`; guest
Direct3D is *designed* to reach the screen through a purpose-built WDDM driver
and a host-side translator onto WebGPU — that path is built but has never run
end to end, and no guest driver has yet been compiled. The
same core also builds as a native runner (`crates/aero-machine-cli`, binary
`aero-machine`) that executes headlessly — that runner, not the browser, is where
bring-up work happens, because it is the fast half of the loop.

Booting that guest to a verified interactive desktop is the project's central
goal. Everything else exists to serve it.

## Whether this has been done before

Not as a whole. No one has booted any 64-bit Windows — or any Windows 7 at all —
to a desktop in a browser. Every individual *layer*, though, is proven somewhere,
which is what separates this from speculation:

| What is proven, and where | Evidence |
|---|---|
| x86 in the browser — but not Windows 7 | v86 boots Windows up to 2000 well and XP/Vista/8 under conditions; Windows 7 is unsupported, with no x64 support and none planned ([v86](https://github.com/copy/v86), [#86](https://github.com/copy/v86/issues/86), [#772](https://github.com/copy/v86/issues/772)) |
| Windows 7 under a hobbyist emulator — but natively, and 32-bit | halfix boots 32-bit Vista/7/10 to a desktop as a native build; its browser demo runs only NT4, OS/2 and Linux ([nepx/halfix](https://github.com/nepx/halfix)) |
| Long mode in the browser — but Linux guests | JSLinux runs x86-64 with SSE4.2/AVX2/AVX-512/APX in-browser and stable, user-measured near 50× slower than native ([tech notes](https://bellard.org/jslinux/tech.html), [report](https://hn.nuxt.dev/item/47311484)) |
| A full PC device model in WebAssembly | qemu-wasm is an upstream-merged Emscripten build with x86_64 guests and MTTCG; the demo guest is Linux ([ktock/qemu-wasm](https://github.com/ktock/qemu-wasm)) |
| D3D → WebGPU translation | **No precedent found.** The proven analogues are all native: DXVK onto Vulkan, DXMT and D3DMetal onto Metal. WGSL tooling exists ([naga](http://kvark.github.io/naga/shader/2022/02/17/shader-translation-benchmark.html)) |

So the GPU path — a custom WDDM driver feeding a WebGPU host — is the only
genuinely unpioneered layer, and it is *optional for first light*: VGA/VBE
reaches a desktop without it. That is why the boot path, not the GPU, is the
critical path.

**What this is for, stated plainly.** Aero is defensible as a flagship research
demonstration — nobody has done it — and it is worth being honest that as a
*product* it would lose to server-side streaming on every axis that matters:
latency, fidelity, battery, engineering cost. That was never the point, and
saying so keeps the scope honest and stops the project acquiring
product-shaped obligations it does not need.

Two consequences worth keeping in view. A 32-bit Windows 7 or XP guest would be
dramatically easier — that is precisely what halfix demonstrates — and remains a
fallback this design does not foreclose, even though the target is x64 SP1. And
because the precedent claims above are a survey of published work, they age:
re-check before relying on "nobody has".
...
> [`areas/cpu-and-jit.md`](./areas/cpu-and-jit.md) (and the [sprint-era record](./history/sprint-era-record.md)).
...
> See [Shared memory layout](decisions/0003-shared-memory-layout.md).
...
- With `MachineConfig::enable_aerogpu=true`, the canonical machine exposes the AeroGPU PCI identity
  ...
  window; see `areas/graphics.md`). Note: the in-tree Win7 AeroGPU driver treats
  ...
  (see also: [`21-emulator-crate-migration.md`](./areas/platform-and-firmware.md)).
- Long-term direction: the AeroGPU WDDM device (`PCI\\VEN_A3A0&DEV_0001`) should be the sole display
  ...
  - [`16-aerogpu-vga-vesa-compat.md`](./areas/graphics.md)
...
- [`specs/worker-ipc-protocol.md`](./specs/worker-ipc-protocol.md)

In code:

- Rust: `crates/aero-ipc/`
- TypeScript: `apps/web/src/ipc/`
...
Browsers cannot reliably allocate a single 5+GiB `SharedArrayBuffer`, and wasm32 linear memory is fundamentally **≤ 4GiB** addressable. To keep the architecture implementable today, Aero uses **Option B: split buffers** (see [Shared memory layout](decisions/0003-shared-memory-layout.md)).

| Buffer | Type | Typical Size | Contents | Access Pattern |
|--------|------|--------------|----------|----------------|
| `guestMemory` | `WebAssembly.Memory` (`shared: true`) | **256 MiB minimum; presets 256/512/1024/2048/3072 MiB** (best-effort) | Guest physical RAM | Shared read/write; the only region directly accessible from wasm today |
...
| `ioIpcSab` | `SharedArrayBuffer` | ~KiB–MiB | High-frequency device IPC rings (CPU/WASM ↔ IO worker, network TX/RX, etc) | AIPC header + Atomics-driven ring buffers (see [`specs/worker-ipc-protocol.md`](./specs/worker-ipc-protocol.md)) |
| `vram` | `SharedArrayBuffer` | **64MiB** (default) | AeroGPU BAR1/VRAM aperture backing (MMIO window; legacy VGA/VBE and optional WDDM surfaces) | I/O worker maps it as an MMIO range; GPU worker reads it for scanout/cursor when `base_paddr` points into the VRAM aperture (see [`areas/graphics.md`](./areas/graphics.md#vram-bar1-backing-as-a-sharedarraybuffer)) |
...
The full definition is in [`specs/worker-ipc-protocol.md`](./specs/worker-ipc-protocol.md). Summary:
...
1. **Read [VM crate map](./state/repo-state-and-structure.md)** for the canonical Rust VM core crate graph (browser + host) and layering rules.
1. **Read [CPU Emulation](./areas/cpu-and-jit.md)** for x86-64 implementation details
2. **Read [Memory Management](./areas/memory-and-paging.md)** for MMU/paging design
3. **Read [Graphics Subsystem](./areas/graphics.md)** for DirectX translation
4. **Review [Task Breakdown](./history/sprint-era-record.md)** for detailed work items
