...
test("IO worker publishes AudioWorklet ring telemetry into StatusIndex.Audio*", async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    const { allocateHarnessSharedMemorySegments } = await import("/apps/web/src/runtime/harness_shared_memory.ts");
    const { StatusIndex, createIoIpcSab, createSharedMemoryViews } = await import("/apps/web/src/runtime/shared_layout.ts");
    ...
    const { requiredBytes: audioRequiredBytes, wrapRingBuffer: wrapAudioRingBuffer } = await import("/apps/web/src/audio/audio_worklet_ring.ts");
    const { openRingByKind } = await import("/apps/web/src/ipc/ipc.ts");
    const { queueKind } = await import("/apps/web/src/ipc/layout.ts");
    const { encodeCommand, decodeEvent } = await import("/apps/web/src/ipc/protocol.ts");
