...
fn translates_structured_srv_index_as_raw_bits_no_float_heuristics() {
    ...
    assert!(
        translated.wgsl.contains("var ld_struct_index1")
            && translated.wgsl.contains("var ld_struct_offset1")
            && translated.wgsl.contains("let ld_struct_base1"),
...
#[test]
fn translates_structured_uav_store_index_as_raw_bits_no_float_heuristics() {
    let dxbc_bytes = dummy_dxbc_bytes();
    let dxbc = DxbcFile::parse(&dxbc_bytes).expect("dummy DXBC parse");
    let signatures = ShaderSignatures::default();
    ...
    let one_f32_bits = src_imm_u32_bits(1.0f32.to_bits());
    let zero = src_imm_u32_bits(0);
    let val_2_f32_bits = src_imm_u32_bits(2.0f32.to_bits());
    let val_3_f32_bits = src_imm_u32_bits(3.0f32.to_bits());
    ...
    let module = Sm4Module {
        ...
        instructions: vec![
            Sm4Inst::StoreStructured {
                ...
                index: one_u32,
                offset: zero.clone(),
                value: val_2_f32_bits,
                mask: WriteMask::XYZW,
            },
            // Same store but with a float-literal immediate (`1.0`). DXBC registers are untyped; the
            // translator must treat this as raw lane bits (0x3f800000), not the integer 1.
            Sm4Inst::StoreStructured {
                uav: UavRef { slot: 0 },
                index: one_f32_bits,
                offset: zero,
                value: val_3_f32_bits,
                mask: WriteMask::XYZW,
            },
            Sm4Inst::Ret,
        ],
    ...
    assert!(
        translated.wgsl.contains("0x40000000u"),
        "expected store value to preserve raw float bits (2.0 -> 0x40000000):\n{}",
        translated.wgsl
    );
    assert!(
        translated.wgsl.contains("0x40400000u"),
        "expected store value to preserve raw float bits (3.0 -> 0x40400000):\n{}",
        translated.wgsl
    );
    assert!(
        translated.wgsl.contains("var store_struct_index1")
            && translated.wgsl.contains("var store_struct_offset1")
            && translated.wgsl.contains("let store_struct_base1"),
        "expected raw-bit structured store address computation to use locals (avoids constant overflow):\n{}",
        translated.wgsl
    );
}
