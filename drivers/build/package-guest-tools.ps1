...
#
# The packaged ISO/zip root may include third-party license/notice texts under `licenses/`
# when present in the source Guest Tools tree.
...
function Get-VersionString {
  # Kept intentionally in sync with drivers/build/package-drivers.ps1 so driver bundle
...
function Normalize-GuestToolsDriverName {
  ...
  $overrides = @{
    ...
    "aero-gpu" = "aerogpu"
    # Support staging from legacy driver bundle layouts (drivers/build/package-drivers.ps1) that use leaf names.
    "blk"      = "virtio-blk"
    "net"      = "virtio-net"
  }
...
function Copy-DriversToPackagerLayout {
  ...
  # CI packages layout produced by `drivers/build/make-catalogs.ps1`:
...
#   pwsh -NoProfile -File drivers/build/package-guest-tools.ps1 -InputRoot $root
#
...
function Assert-GuestToolsZipContainsAeroGpuDbgctl {
  ...
  $expectedEntries = @(
    # `drivers/build/build-aerogpu-dbgctl.ps1` stages dbgctl into:
    #   out/drivers/aerogpu/<arch>/tools/win7_dbgctl/bin/aerogpu_dbgctl.exe
    # `drivers/build/make-catalogs.ps1` then copies build outputs into out/packages/... preserving the `tools/` folder.
...
To fix:
  - Ensure the dbgctl build step runs and stages the binary (see: drivers/build/build-aerogpu-dbgctl.ps1).
