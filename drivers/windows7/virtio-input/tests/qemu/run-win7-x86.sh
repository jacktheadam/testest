#!/bin/sh
#
# QEMU helper: Windows 7 SP1 x86 virtio-input bring-up (virtio keyboard + mouse).
#
...
#
# Usage:
#   ./run-win7-x86.sh [--multifunction] [--i8042-off] [--vectors N] <disk-image> [--] [<extra-qemu-args...>]
#
set -eu
...
Environment overrides:
  QEMU_BIN=...         Override qemu-system-i386.
...
if ! command -v "$qemu_bin" >/dev/null 2>&1; then
  ...
  echo "hint: install QEMU and/or set QEMU_BIN=/path/to/qemu-system-i386" >&2
  exit 2
fi
...
#
# Override with:
#   QEMU_DISK_FORMAT=qcow2 ./run-win7-x86.sh ...
