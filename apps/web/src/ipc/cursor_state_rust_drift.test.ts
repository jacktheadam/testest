...
function getAerogpuFormatValues(): Readonly<Record<string, bigint>> {
  ...
  const srcUrl = new URL("../../../../crates/aero-protocol/aerogpu/aerogpu_pci.rs", import.meta.url);
  const src = readFileSync(srcUrl, "utf8");

  const enumMatch = src.match(/pub enum AerogpuFormat\s*\{([\s\S]*?)\r?\n\}/m);
  if (!enumMatch) {
    throw new Error("Failed to locate `pub enum AerogpuFormat { ... }` in crates/aero-protocol/aerogpu/aerogpu_pci.rs");
  }
