...
**Decision (0012).** `Cargo.lock` is committed and builds pass `--locked`, so a build resolves the same dependencies today as it did last month.
...
- The repository **must** contain an up-to-date lockfile for:
  ...
  - Standalone nested workspaces, which now means exactly one: `fuzz/Cargo.lock`,
    separate because cargo-fuzz requires its own workspace and toolchain.

    The tools that once had their own workspaces — the disk gateway, the image
    chunker, the slipstreamer, the packager and the certificate-store tools —
    have all joined the root workspace, so their lockfiles are gone. One dead
    file survives that consolidation and should be deleted:
    `tools/win-offline-cert-injector/Cargo.lock`, whose crate is a root member,
    so the lockfile is never consulted.
