...
- for chunks without `sha256`, size-only verification is optimized (best-effort) to avoid downloading chunk bodies (via `HEAD` or `Range` requests + `Content-Length`/`Content-Range` checks)
...
- **HTTP/CDN**: `--manifest-url https://.../manifest.json` (fetches `chunks/*.bin` relative to the manifest URL)
  ...
  - For chunks without `sha256`, the verifier may send `HEAD` (or `Range` `GET`) requests to validate size without downloading full bodies (it falls back to `GET` if unsupported/blocked)
