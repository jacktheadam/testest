# Direct3D 9Ex and the Windows 7 desktop compositor

> Windows 7's desktop compositor is a Direct3D 9Ex client, so D3D9Ex is the
> interface that decides whether the desktop composes at all. This page
> specifies the guest-side surface Aero must present, the host-side protocol
> that backs it, and the fence contract that keeps composition from stalling.
...
### Resource behaviors Ex clients rely on
...
Define a guest/host sharing model that does **not** attempt to expose host OS handles:
  ...
  - This value is typically a normal Windows handle (**NT handle**): **process-local**, not stable cross-process, and commonly different in the consumer after `DuplicateHandle`.
    - Some D3D9Ex implementations use “token-style” shared handles that are not real NT handles and cannot be duplicated (and should not be passed to `CloseHandle`); even in that case, the numeric value is not a stable protocol key.
...
See `aerogpu-device-abi.md` for implementation details and the full Win7 cross-process
...
### Fence/completion signaling (host → guest)
