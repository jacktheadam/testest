//! Sample-format, sample-rate, and channel-layout conversion.
//!
//! A guest picks its own stream format and the host picks its own; a Windows 7 title may open a
//! 22.05 kHz mono stream while the browser's audio context runs 48 kHz stereo. Everything needed to
//! get from one to the other lives here: [`pcm`] decodes the guest's sample format, [`remix`]
//! reconciles channel counts, [`resample`] reconciles rates, and [`mix`] combines streams.
//!
//! [`StreamProcessor`] chains those steps in the order that keeps the most information — decode,
//! resample, then remix — so a downmix never discards detail the resampler could have used.
