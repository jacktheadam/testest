...
drivers/aerogpu/tests/win7/
  ...
  d3d11_geometry_shader_restart_strip/
...
* `d3d9_caps_smoke` sanity-checks key `D3DCAPS9` bits reported by `IDirect3DDevice9Ex::GetDeviceCaps` (windowed rendering, shareable resources, shader model level, HW T&L, scissor/compare capabilities, NPOT textures) and validates patch cap advertisement (requires `D3DDEVCAPS_RTPATCHES`, disallows `D3DDEVCAPS_NPATCHES` / `D3DDEVCAPS_QUINTICRTPATCHES`, and requires `MaxNpatchTessellationLevel > 0`)
* `d3d9ex_fixedfunc_state_roundtrip` validates D3D9Ex fixed-function state caching and `Set*`/`Get*` roundtrips (`SetTransform`/`GetTransform`, `SetTextureStageState`/`GetTextureStageState`) and exercises `IDirect3DStateBlock9` record/apply restore behavior for these cached states (no rendering required)
...
* `perf_state_sanity` queries the KMD `QUERY_PERF` escape repeatedly and validates basic invariants, including that the appended `flags` field is present and has the `VALID` bit set. On newer KMD builds it also validates appended counters such as GetScanLine telemetry, contig-pool hit/miss/bytes-saved, and alloc-table count/entries/readonly_entries (skips if the escape is not supported)
...
* `cursor_state_sanity` moves the cursor, sets a custom cursor shape, and queries cursor MMIO state via `AEROGPU_ESCAPE_OP_QUERY_CURSOR` (validates cursor enable/position/hotspot + that shape fields are non-zero; skips if cursor escapes/MMIO are not supported). When run with `--json`, it also emits a `cursor_state_sanity_diag.txt` artifact with cursor query snapshots.
* `modeset_roundtrip_sanity` switches to an alternate supported desktop mode (when available), validates scanout state matches it, and switches back (catches broken VidPN pruning/recommendation and `DxgkDdiCommitVidPn` bookkeeping/mode-change regressions; fails under RDP unless `--allow-remote` is passed to skip; skips if the scanout escape is not supported)
...
* `map_shared_handle_stress` stress-tests `AEROGPU_ESCAPE_OP_MAP_SHARED_HANDLE`: validates the same section handle maps to a stable non-zero debug token across many iterations and that the driver remains responsive under many unique section handles (cap/eviction behavior is accepted; skips if the escape is not supported or is gated off)
...
* `dbgctl_escape_security_sanity` provides negative coverage for security-sensitive dbgctl escapes (`READ_GPA`, `MAP_SHARED_HANDLE`), ensuring invalid GPAs/handles are rejected and do not copy any bytes (skips when the escapes are not supported or are gated off)
* `dbgctl_selftest_sanity` runs the KMD dbgctl selftest escape (`AEROGPU_ESCAPE_OP_SELFTEST`) and expects it to report PASS; retries briefly on `GPU_BUSY` and may skip with `gpu_busy` if the adapter remains busy (skips if the escape is not supported)
* `alloc_table_readonly_sanity` produces an AGPU submission and reads back its per-submit allocation table via dbgctl `READ_GPA`, validating that it contains both `AEROGPU_ALLOC_FLAG_READONLY` and writable entries (verifies propagation of WDDM submission-time write metadata (`WriteOperation` bit; `DXGK_ALLOCATIONLIST::Flags.Value & 0x1`) into `alloc_table.flags`; on newer KMD builds it also cross-checks that the `QUERY_PERF` alloc-table counters increase; skips on non-AGPU rings; requires `DUMP_RING_V2` + `READ_GPA`)
...
* `d3d9_process_vertices_sanity` validates `IDirect3DDevice9::ProcessVertices` by reading back the transformed vertex bytes from a system-memory destination VB and comparing against expected results (covers pass-through XYZRHW and deterministic fixed-function XYZ→XYZRHW conversion paths)
* `d3d9_mipmapped_texture_smoke` uploads a small mipmapped texture, renders with it, and validates mip selection/sampling via readback; it also validates per-mip `LockRect` pitch reporting on a SYSTEMMEM mip chain and the `UpdateTexture(SYSTEMMEM→DEFAULT)` upload path (sampling all mips via `D3DSAMP_MAXMIPLEVEL`) (sanity-check for basic D3D9 mip upload + sampling paths)
* `d3d9_fixedfunc_xyz_diffuse` draws a fixed-function triangle using `D3DFVF_XYZ|D3DFVF_DIFFUSE` with world/view/projection transforms and validates output via backbuffer readback (expects **corner clear + center diffuse**, within tolerance)
* `d3d9_fixedfunc_xyz_diffuse_tex1` draws a fixed-function textured triangle using `D3DFVF_XYZ|D3DFVF_DIFFUSE|D3DFVF_TEX1` and validates that the modulated diffuse×texture colors appear at four sample points via readback (catches fixed-function texturing regressions)
* `d3d9_fixedfunc_multitexture` validates multi-texture stage chaining: draws a fullscreen quad with two point-sampled 1×1 textures and uses stage0 `SELECTARG1=TEXTURE`, stage1 `MODULATE(TEXTURE×CURRENT)` to produce a deterministic modulated color; validates the center pixel via readback (within tolerance). Supports `--dump` to emit `d3d9_fixedfunc_multitexture.bmp`.
* `d3d9_fixedfunc_fog_smoke` enables fixed-function linear fog (`D3DRS_FOG*`) and draws two fixed-function quads at different depths using `D3DFVF_XYZRHW|D3DFVF_DIFFUSE`; validates via readback that the near quad is mostly diffuse and the far quad trends toward `FOGCOLOR` (within tolerance)
* `d3d9_fixedfunc_vertexdecl_patterns` validates fixed-function vertex declarations with non-canonical element ordering (`TEXCOORD0` before `COLOR0`): draws a textured quad using stage0 `D3DTOP_MODULATE` (TEXTURE×DIFFUSE) and confirms the expected 2×2 texture pattern appears at four sample points via readback (supports `--dump` to write `d3d9_fixedfunc_vertexdecl_patterns.bmp` and `d3d9_fixedfunc_vertexdecl_patterns.bin`).
* `d3d9_fixedfunc_lighting_directional` validates fixed-function directional lighting (HW T&L): renders the same geometry twice with opposite `D3DLIGHT_DIRECTIONAL` directions and requires that the center pixel is significantly brighter for the “lit” direction and near-black for the “dark” direction (failure implies broken fixed-function lighting/normal handling; supports `--dump` to emit `d3d9_fixedfunc_lighting_directional_{lit,dark}.bmp` and `*.bin` dumps for debugging).
* `d3d9_fixedfunc_lighting_multi_directional` validates fixed-function multi-light accumulation: renders once with a red `D3DLIGHT_DIRECTIONAL`, then again with an additional green directional light, and requires that the center pixel gains significant green (red → yellow) (supports `--dump` to write `d3d9_fixedfunc_lighting_multi_directional_{red,red_green}.{bmp,bin}`).
* `d3d9_fixedfunc_lighting_point` validates fixed-function point lighting and attenuation: renders a quad lit by a point light near the center and requires that the center pixel is significantly brighter than a corner sample (supports `--dump` to write `d3d9_fixedfunc_lighting_point.{bmp,bin}`).
* `d3d9_patch_rendering_smoke` exercises the D3D9 patch path (`DrawRectPatch` / `DeletePatch` / `DrawTriPatch`, including handle=0 no-cache paths, cache-hit draw-twice paths, redraw after `DeletePatch`, and a `D3DFVF_TEX1` variant) and validates that the patch outputs expected colors over a clear background via backbuffer readback (skips if patch caps are not advertised)
* `d3d9_patch_sanity` probes D3D9 patch caps and (when supported) creates and renders cubic Bezier tri and rect patches and validates **corner red + center blue/green** via readback (otherwise the test is skipped)
...
* `d3d9ex_ps_only_triangle` binds a simple `ps_2_0` passthrough pixel shader with **NULL vertex shader** (fixed-function vertex processing), renders a blue triangle over a red clear, and validates via readback: **corner red + center blue** (catches pixel-shader pipeline / color channel-order regressions independently of vertex shader bring-up)
* `d3d9_fixedfunc_wvp_triangle` draws two fixed-function triangles to validate WORLD/VIEW/PROJECTION transform application and `IDirect3DStateBlock9::Apply()` ordering: a first **blue** triangle covers the center pixel under identity transforms, then a state block applies a WORLD translate + PROJECTION X-scale to move the second **green** triangle to the right (expects **corner red + center blue + right green** via readback); also exercises `GetPresentStats`
* `d3d9ex_instancing_sanity` validates D3D9Ex instancing via `SetStreamSourceFreq` (`D3DSTREAMSOURCE_INDEXEDDATA` / `D3DSTREAMSOURCE_INSTANCEDATA`) by drawing two instanced triangles using a per-instance VB (instance0 = red on the left, instance1 = green on the right) and validating pixels via readback (**left red + right green + corner clear**). With `--dump` it writes `d3d9ex_instancing_sanity.bmp`.
...
* `d3d9_fixedfunc_textured_wvp` validates fixed-function WORLD/VIEW/PROJECTION transforms for textured geometry by applying non-identity WORLD/VIEW/PROJECTION transforms (WORLD/VIEW translation plus a PROJECTION scale+translation) and confirming the center pixel matches the expected **stage0 modulated** texture color (TEXTURE×DIFFUSE) via readback. It also toggles transforms to identity between draws to ensure the fixed-function WVP path observes transform changes (center pixel returns to clear color), and covers both `SetFVF` and `SetVertexDeclaration` fixed-function paths. With `--dump`, it writes per-phase backbuffer dumps as both `*.bmp` and tight BGRA32 `*.bin`.
* `d3d9_shader_stage_interop` validates D3D9 stage binding interop by drawing with a user `vs_2_0` vertex shader + **NULL pixel shader** while forcing stage0 to select the bound texture (expects **corner clear red + center magenta** via readback)
...
* `d3d9_process_vertices_smoke` calls `IDirect3DDevice9::ProcessVertices` with non-zero `SrcStartIndex`/`DestIndex` (and a non-zero stream offset), then draws from the processed vertex buffer and confirms via readback: **corner clear + center blue**, plus sentinel triangles (**green** + **yellow**) to catch `DestIndex` bugs and out-of-bounds writes (supports `--dump` to emit `d3d9_process_vertices_smoke.bmp`, `d3d9_process_vertices_smoke.bin` (tight BGRA32), and `d3d9_process_vertices_smoke_vb.bin` (raw processed VB bytes))
* `d3d9_dynamic_vb_lock_semantics` exercises D3D9 dynamic vertex buffer `Lock(D3DLOCK_DISCARD)`/`Lock(D3DLOCK_NOOVERWRITE)` semantics by drawing multiple triangles; DISCARD renaming must not overwrite in-flight data, and NOOVERWRITE appends must preserve previously written vertices (validated via render-target readback)
...
* `d3d9ex_shared_surface_ipc` creates a shared D3D9Ex render-target texture in one process, opens it in a second process, and validates the consumer can read back the producer’s clear color.
  * If the shared handle is a real NT handle, the producer duplicates it into the consumer (best-effort avoiding a numeric handle collision across processes) and (when supported) confirms `AEROGPU_ESCAPE_OP_MAP_SHARED_HANDLE` maps both process-local handles to the same stable **debug token**.
  * If the shared handle is not a real NT handle, it falls back to passing the raw handle value (token-style shared handles; do not call `CloseHandle` on token values).
...
* `d3d10_rs_om_state_sanity` validates D3D10 rasterizer + output-merger state correctness (scissor enable, cull mode/front-face, depth clip, depth comparisons, alpha blending + write mask + blend factor + sample mask, plus ClearState reset behavior including primitive topology) via offscreen readback
...
* `d3d10_1_rs_om_state_sanity` validates D3D10.1 rasterizer + output-merger state correctness (scissor enable, cull mode/front-face, depth clip, depth comparisons, alpha blending + write mask + blend factor + sample mask, plus ClearState reset behavior including primitive topology) via offscreen readback
...
* `d3d11_texture_mips_array_sanity` renders a 2x2 quad sampling a `Texture2DArray` with 2 slices and 2 mips via `SampleLevel`, validating **slice/mip selection** (expects a 2x2 readback of **red, green, blue, white**; requires feature level >= 10_0)
* `d3d11_caps_smoke` validates the expected D3D11 feature level and common format support bits used by the runtime
* `d3d11_rs_om_state_sanity` validates D3D11 rasterizer + blend state correctness (scissor enable/disable + `RSSetState(NULL)`, cull mode/front-face, depth clip enable/disable, alpha blending + write mask + blend factor + sample mask, plus ClearState reset behavior including primitive topology) via readback (requires feature level >= 10_0)
* `d3d11_geometry_shader_smoke` renders a triangle through the Geometry Shader stage (requires feature level >= 10_0) and confirms **corner red + center green** via readback. With `--dump` it also writes the compiled VS/GS/PS bytecode (`d3d11_geometry_shader_smoke_{vs,gs,ps}.dxbc`) for debugging shader translation/emulation issues.
* `d3d11_geometry_shader_restart_strip` validates `TriangleStream.RestartStrip()` (`cut`) semantics when the GS outputs multiple triangle strips (two quads emitted from a single invocation) that must be converted into a triangle list (expects **two disjoint quads** with the center gap remaining clear). With `--dump` it also writes the compiled VS/GS/PS bytecode (`d3d11_geometry_shader_restart_strip_{vs,gs,ps}.dxbc`).
