...
#[test]
fn pc_platform_virtio_blk_uses_injected_disk_for_reads_and_writes() {
    ...
    let mut disk = RawDisk::create(MemBackend::new(), DISK_SECTORS * VIRTIO_BLK_SECTOR_SIZE)
        .expect("failed to allocate in-memory virtio-blk disk");
    disk.write_at(TEST_SECTOR * VIRTIO_BLK_SECTOR_SIZE, &[9, 8, 7, 6])
        .unwrap();
    ...
    // --- Request 0: READ sector `TEST_SECTOR` (should see the seeded bytes). ---
    ...
    let mut got = [0u8; 4];
    pc.memory.read_physical(data, &mut got);
    assert_eq!(got, [9, 8, 7, 6]);

    // Clear ISR to deassert the legacy interrupt level before the next request.
    let _ = pc.memory.read_u8(bar0_base + ISR);

    // --- Request 1: WRITE new bytes into the same sector, then read them back. ---
    let mut sector = [0u8; VIRTIO_BLK_SECTOR_SIZE as usize];
    sector[0..4].copy_from_slice(&[1, 2, 3, 4]);
    pc.memory.write_physical(data, &sector);
    ...
    // --- Request 3: READ beyond the injected disk capacity should return IOERR. ---
    let _ = pc.memory.read_u8(bar0_base + ISR);
    pc.memory.write_u32(header, VIRTIO_BLK_T_IN);
    pc.memory.write_u32(header + 4, 0);
    ...
    pc.memory.write_u16(AVAIL_RING + 10, 0);
    pc.memory.write_u16(AVAIL_RING + 2, 4);
    pc.memory.write_u16(bar0_base + NOTIFY, 0);
    pc.process_virtio_blk();
...
#[test]
fn pc_platform_virtio_blk_multi_descriptor_read_write_roundtrip() {
    const RAM_SIZE: usize = 2 * 1024 * 1024;
    const DISK_SECTORS: u64 = 8;

    // Seed two consecutive sectors so a 2-sector READ (with multiple descriptors) can validate that
    // the device streams data across segments in order.
    let mut disk = RawDisk::create(MemBackend::new(), DISK_SECTORS * VIRTIO_BLK_SECTOR_SIZE)
        .expect("failed to allocate in-memory virtio-blk disk");
    ...
    // --- Request 0: READ 2 sectors (sector 0 + 1) into two data descriptors. ---
    pc.memory.write_u32(header, VIRTIO_BLK_T_IN);
    pc.memory.write_u32(header + 4, 0);
    ...
    write_desc(&mut pc, DESC_TABLE, 3, status, 1, VIRTQ_DESC_F_WRITE, 0);
    ...
    // --- Request 1: WRITE 2 sectors using two data descriptors, then read them back. ---
    ...
    // Read back using the same multi-descriptor layout.
    pc.memory
        .write_physical(data0, &[0u8; VIRTIO_BLK_SECTOR_SIZE as usize]);
    pc.memory
        .write_physical(data1, &[0u8; VIRTIO_BLK_SECTOR_SIZE as usize]);
...
#[test]
fn pc_platform_virtio_blk_msix_function_mask_defers_delivery_until_unmasked() {
    ...
    // Enable BAR0 MMIO decode + bus mastering so we can access BAR0 and allow DMA.
    write_cfg_u16(&mut pc, bdf.bus, bdf.device, bdf.function, 0x04, 0x0006);
    ...
    // Discover MSI-X table/PBA offsets.
    let table = read_cfg_u32(
        &mut pc,
        bdf.bus,
        bdf.device,
    ...
    // Program table entry 0 with an xAPIC message targeting vector 0x65.
    let vector = 0x65u32;
    let table_base = bar0_base + u64::from(table & !0x7);
    ...
    // rather than delivered while Function Mask is set.
    pc.memory.write_u16(bar0_base + NOTIFY, 0);
    pc.process_virtio_blk();
    assert_eq!(pc.memory.read_u16(USED_RING + 2), 1);
    assert_eq!(pc.memory.read_u8(status), 0);
    ...
    assert!(
        !pc.virtio_blk.as_ref().unwrap().borrow().irq_level(),
        "legacy INTx should not be asserted once MSI-X is enabled (even if masked)"
    );
    assert_eq!(
...
#[test]
fn pc_platform_virtio_blk_snapshot_restore_with_injected_disk_processes_pending_read_without_renotify(
) {
    const RAM_SIZE: usize = 2 * 1024 * 1024;
    const DISK_SECTORS: u64 = 8;
    const TEST_SECTOR: u64 = 3;
    ...
    // Build a minimal READ request for `TEST_SECTOR`.
    const VIRTIO_BLK_T_IN: u32 = 0;
    const VIRTQ_DESC_F_NEXT: u16 = 0x0001;
    const VIRTQ_DESC_F_WRITE: u16 = 0x0002;
    ...
    // Restore into a fresh platform with the same disk contents.
    let mut disk2 = RawDisk::create(MemBackend::new(), DISK_SECTORS * VIRTIO_BLK_SECTOR_SIZE)
        .expect("failed to allocate in-memory virtio-blk disk");
    disk2
        .write_at(TEST_SECTOR * VIRTIO_BLK_SECTOR_SIZE, &[9, 8, 7, 6])
        .unwrap();
    ...
    let mut first4 = [0u8; 4];
    restored.memory.read_physical(data, &mut first4);
    assert_eq!(first4, [9, 8, 7, 6]);
