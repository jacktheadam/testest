...
describe("keyboardCodeToHidUsage", () => {
  it("matches the shared fixture (protocol-vectors/hid_usage_keyboard.json)", () => {
    const fixtureUrl = new URL("../../../../protocol-vectors/hid_usage_keyboard.json", import.meta.url);
...
describe("keyboardCodeToConsumerUsage", () => {
  it("matches the shared fixture (protocol-vectors/hid_usage_consumer.json)", () => {
    const fixtureUrl = new URL("../../../../protocol-vectors/hid_usage_consumer.json", import.meta.url);
    const entries = JSON.parse(readFileSync(fixtureUrl, "utf8")) as FixtureEntry[];
    expect(Array.isArray(entries)).toBe(true);
    expect(entries.length).toBeGreaterThan(0);
