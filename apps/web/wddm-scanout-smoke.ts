  ...
  try {
    ...
    const runContextLossRecovery = async () => {
      ...
      try {
        ...
        // Without the worker's own event trail this failure says only that nothing arrived.
        return {
          ok: false,
          error: formatOneLineError(err, 512),
          loseOk,
          restoreOk,
          recentEvents: gpuEvents
            .slice(debugRestoreIndex)
            .map(
              (e) =>
                `${String(e?.category)}:${String(e?.message).slice(0, 90)}` +
                (e?.details ? ` details=${JSON.stringify(e.details).slice(0, 220)}` : ""),
            )
            .slice(0, 12),
        };
      }
      ...
      return {
        ...
        after: {
          ...
          // A hash mismatch with correct corner samples means the presenter came back at a
          // different size, so report the dimensions alongside the hash.
          width: presentedAfterWidth,
          height: presentedAfterHeight,
          expectedWidth: presentedWidth,
          expectedHeight: presentedHeight,
        },
      };
    };
