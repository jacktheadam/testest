...
**Decision (0008).** `aero-machine` is the canonical VM core. There is one crate that defines the stable `Machine` API, and everything browser-facing goes through it.

> Note: The broader "canonical machine stack" decision is also captured in
> [Canonical machine stack](0014-canonical-machine-stack.md). This ADR focuses on consolidating
...
### Stable public API surface (minimum)
...
### WASM/browser-facing VM wrappers (current state)
...
- **`WasmVm` / `WasmTieredVm`** (legacy CPU-worker runtime): CPU-only stepping loops used by
  `apps/web/src/workers/cpu.worker.ts` (`vmRuntime=legacy`). They execute CPU in WASM but forward port I/O + MMIO back to JS
  ...
  - In `vmRuntime=machine`, the CPU worker entrypoint is `apps/web/src/workers/machine_cpu.worker.ts` and it drives `Machine`.
...
- [`wiki/areas/platform-and-firmware.md`](../areas/platform-and-firmware.md) (canonical vs legacy crate map)
- [`wiki/areas/platform-and-firmware.md`](../areas/platform-and-firmware.md) (`crates/emulator` → canonical stack plan + deletion targets)
- [Canonical machine stack](0014-canonical-machine-stack.md) (canonical machine stack)
...
See also: [`wiki/areas/platform-and-firmware.md`](../areas/platform-and-firmware.md) (canonical vs legacy crate map; `crates/emulator` → canonical stack plan + deletion targets).
