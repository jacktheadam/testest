  ...
  test("dynamic vhd -> aerosparse + logical bytes preserved", async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      function buildDynamicVhdFixture(): { file: Uint8Array; logical: Uint8Array } {
        ...
        writeU64BE(footer, 40, BigInt(logicalSize)); // original_size
        ...
        writeU64BE(dyn, 8, 0xffff_ffff_ffff_ffffn); // data_offset (u64::MAX)
        writeU64BE(dyn, 16, BigInt(batOffset));
        writeU32BE(dyn, 24, 0x0001_0000); // header_version
        writeU32BE(dyn, 28, 1);
