...
This writes a JSON report to `services/gateway/bench/results.json` and prints a human-readable summary to stdout.
...
node services/gateway/bench/run.mjs --mode nightly --json services/gateway/bench/results.json
```
...
```bash
npm -w services/gateway run bench:smoke
```
