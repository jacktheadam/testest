...
.DESCRIPTION
  ...
  The script:
    - Loads out/toolchain.json (emitted by drivers/build/install-wdk.ps1), or falls back to
...
try {
  ...
  if ($null -ne $toolchain) {
    # drivers/build/install-wdk.ps1 may emit different JSON schemas depending on environment/tooling.
