import type { AeroGlobalApi } from '../../../packages/shared/aero_api.ts';
...
declare global {
  ...
  interface Window {
    ...
    /**
     * E2E-visible state for the legacy `bringup.html` canonical Machine panel.
     */
    ...
    /**
     * Optional SharedArrayBuffer-backed framebuffer published by the legacy `bringup.html`
    ...
    /**
     * E2E-visible state for the `bringup.html` canonical Machine worker VGA demo panel.
     */
  ...
  /**
   * The same namespace reached through `globalThis` rather than `window`.
   *
   * Worker code has no `window` and goes through `globalThis`, and so does the host shell in a few
   * places. Declaring the variable from the `Window` member keeps both paths on one shape: there is
   * no second declaration to drift, and no `any` standing in for the typed API.
   */
