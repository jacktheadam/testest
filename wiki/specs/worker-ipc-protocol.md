# Worker IPC protocol and shared-memory layout

> The binary protocol between the coordinator and the emulator's workers, and
> the shared-memory segments it operates on: ring structure, record framing,
> wake-up rules, and the layout of each shared buffer.
...
`apps/web/src/runtime/shared_layout.ts:createIoIpcSab()`), the following kinds are currently assigned:
...
- **Expected max frame size:** `2048` bytes.
  ...
  - Rationale: matches the default L2 tunnel payload limit (`apps/web/src/shared/l2TunnelProtocol.ts`) and
...
See `apps/web/src/hid/hid_input_report_ring.ts`.
...
## Concurrency semantics (MPSC-safe)
...
## Message protocol (record payloads)
...
- TypeScript: `apps/web/src/ipc/{layout,ring_buffer,protocol}.ts`
- TypeScript (layout builder/parser): `apps/web/src/ipc/ipc.ts`
- Browser demo: `apps/web/demo/ipc_demo.html`
