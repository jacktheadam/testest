...
import { formatOneLineError, formatOneLineUtf8, truncateUtf8 } from "../../../../packages/transport-safety/src/text.js";
...
const MAX_UI_ERROR_STACK_BYTES = 8 * 1024;
...
function serializeUiErrorCause(cause: unknown): unknown {
  ...
  const safeStack = stack ? truncateUtf8(stack, MAX_UI_ERROR_STACK_BYTES) : undefined;
  return { name: safeName, message, ...(safeStack ? { stack: safeStack } : {}) };
}
...
import { AerogpuCmdStreamIter } from "../../../../crates/aero-protocol/aerogpu/aerogpu_cmd.ts";
import { AerogpuFormat, aerogpuFormatToString } from "../../../../crates/aero-protocol/aerogpu/aerogpu_pci.ts";
...
function serializePresenterErrorForPostMessage(
  err: PresenterError,
): { name: string; code: string; message: string; stack?: string; cause?: unknown } {
  const base = serializeErrorForPostMessage(err);
  const code = formatOneLineUtf8(err.code, MAX_UI_ERROR_NAME_BYTES) || "unknown";
  return { ...base, code };
}
...
function sanitizeForPostMessage(value: unknown): unknown {
  ...
  if (value instanceof PresenterError) {
    return serializePresenterErrorForPostMessage(value);
  }
  if (value instanceof Error) {
    return serializeErrorForPostMessage(value);
  }
    ...
    return JSON.parse(
      JSON.stringify(value, (_key, v) => {
        ...
        if (v instanceof PresenterError) {
          return serializePresenterErrorForPostMessage(v);
        }
        if (v instanceof Error) {
          return serializeErrorForPostMessage(v);
        }
        return v as unknown;
      }),
    );
...
function postFatalError(err: unknown): void {
  if (err instanceof PresenterError) {
    ...
    postToMain({ type: "error", message, code: err.code, backend: presenter?.backend });
...
// any lingering legacy dirty flags (see wiki/areas/graphics.md#legacy-vgavbe-compatibility-and-the-boot--wddm-scanout-handoff).
//
...
const presentOnce = async (): Promise<boolean> => {
  ...
  try {
    ...
    if (presenter) {
      ...
      // output after WDDM has claimed scanout, matching `wiki/areas/graphics.md#legacy-vgavbe-compatibility-and-the-boot--wddm-scanout-handoff`.
      if (wddmOwnsScanout) {
        const hasBasePaddr =
    ...
    if (shouldUseWasmExecutor) {
      ...
      try {
        ...
        emitGpuEvent({
          ...
          message: formatUiErrorMessage(err),
          details: serializeUiErrorDetails(err),
        });
        runCpu();
      }
...
function postPresenterError(err: unknown, backend?: PresenterBackendKind): void {
  ...
  let message = formatUiErrorMessage(err);
...
async function tryInitBackend(
  ...
  // A WebGL context loss leaves the presenter in place (see `handleDeviceLost`) so a restore can
  // reuse it, and recovery must not begin until the backend has rebuilt the objects it owns —
  // anything touching a texture from the dead context fails with INVALID_OPERATION. The backend
  // tells us when it is ready; that signal, not the canvas event, is what drives recovery.
    ...
    case "debug_context_loss": {
      ...
      try {
        ...
        emitGpuEvent({
          ...
          severity: "warn",
          category: "Debug",
          message: `debug_context_loss failed: ${formatUiErrorMessage(err)}`,
        });
      }
      break;
    }
