...
> Guest physical memory, the page-table walker and software TLB, the guest
> physical address map with its PCI hole and high-RAM remap, and the memory
> backends that serve them. The execution engine that drives these structures
> is covered in [cpu-and-jit.md](./cpu-and-jit.md).
...
See: [AeroGPU Legacy VGA/VBE Compatibility](graphics.md)
and [AeroGPU PCI identity](../specs/aerogpu-device-abi.md) (AeroGPU vs standalone VGA/VBE path).
...
- See [CPU Emulation](cpu-and-jit.md) for instruction implementation
- See [Graphics Subsystem](graphics.md) for video memory
- See [Storage Subsystem](storage.md) for disk I/O
