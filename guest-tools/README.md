...
For diagnostics, `uninstall.cmd` also logs the Guest Tools manifest metadata (version/build_id) and `signing_policy`. It searches for `manifest.json` next to the script directory first, and falls back to the media root (one directory above the script directory) if needed (same behavior as `setup.cmd`).  
