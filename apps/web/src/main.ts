...
import { createAudioOutput } from '../../web/src/platform/audio';
import { detectPlatformFeatures, explainMissingRequirements, type PlatformFeatureReport } from '../../web/src/platform/features';
import { importFileToOpfs } from '../../web/src/platform/opfs';
import { requestWebGpuDevice } from '../../web/src/platform/webgpu';
...
function renderRemoteDiskPanel(): HTMLElement {
  ...
  const warning = el(
    ...
    'The server must support either HTTP Range requests (single-file images) or the chunked manifest format (see wiki/areas/storage.md).',
  );
...
function renderPerfPanel(report: PlatformFeatureReport): HTMLElement {
  ...
  // The panel reports which halves of the perf API are actually installed. The checks stay at
  // runtime because `installAeroGlobal` populates this namespace after the shell renders, and a
  // build that omits the HUD leaves parts of it absent.
  const perfApi = globalThis.aero?.perf;

  const hasCaptureApi =
    typeof perfApi?.captureStart === 'function' &&
    typeof perfApi?.captureStop === 'function' &&
    typeof perfApi?.export === 'function';

  const hasTraceApi =
    typeof perfApi?.traceStart === 'function' &&
    typeof perfApi?.traceStop === 'function' &&
    typeof perfApi?.exportTrace === 'function';
