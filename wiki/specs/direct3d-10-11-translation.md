# Direct3D 10/11 translation

> How Direct3D 10/11 concepts — the fixed pipeline, the resource and view
> model, constant buffers, input layouts, state objects, and SM4/SM5 bytecode —
> map onto WebGPU and WGSL, including the binding model that every translated
> stage shares.
>
> Stages WebGPU lacks entirely are specified separately in
> [compute-expansion-emulation.md](./compute-expansion-emulation.md).
...
- Expansion-specific internal buffers (vertex pulling inputs, scratch outputs, counters, indirect
  ...
  - Implementation detail: the in-tree vertex pulling WGSL uses `@group(3)` (see
    ...
    without collisions.
  See [`compute-expansion-emulation.md`](compute-expansion-emulation.md).
...
### AeroGPU ABI extensions for GS/HS/DS
...
`resolve_shader_stage_with_ex_gated`; see `crates/aero-protocol/aerogpu/aerogpu_cmd.rs`).
...
### Compute-expansion runtime pipeline
...
### Binding model for emulation kernels
...
1. **Protocol decoding/encoding**
   ...
   - Location: `crates/aero-protocol/tests/*` (Rust + TS mirrors).
...
- `../areas/graphics.md` (overall graphics architecture and DXBC→WGSL flow)
- `windows7-d3d-umd-ddi.md` (Win7 UMD DDI surface: entrypoints, feature levels, swapchain/present expectations)
- `windows7-d3d-umd-ddi.md` (Win7 D3D11 `d3d11umddi.h` function-table checklist: REQUIRED vs stubbable for FL10_0 bring-up)
- `../areas/testing.md` (how pixel-compare tests fit into CI)
- `../history/project-history.md` (task-level execution plan)
