# Split virtqueues on Windows 7

> The virtio 1.0 split-ring mechanics as implemented by the Windows 7 KMDF
> drivers: descriptor tables, available and used rings, notification
> suppression, memory-barrier placement, and the DMA constraints the driver
> framework imposes.
...
* [`windows7-virtio-driver-contract.md`](windows7-virtio-driver-contract.md) — DMA/common-buffer strategy for split rings + indirect tables on Win7.
* [`../areas/windows-drivers.md`](../areas/windows-drivers.md) — virtio-pci modern interrupts on Win7 (MSI-X vs INTx).
...
## Split ring layout and field semantics
...
## Ring memory layout, sizing, and alignment
...
## Driver-side virtqueue state (what to track)
...
## Descriptor allocation/free: free list + cookie mapping
...
## Publishing to the avail ring (driver → device)
...
## Consuming the used ring (device → driver)
...
## Windows 7 / KMDF specifics (practical constraints)
...
## End-to-end pseudocode example (virtio-input style RX queue)
...
## Common pitfalls (things that break real drivers)
