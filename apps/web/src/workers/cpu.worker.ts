...
} from "../../../../crates/aero-protocol/aerogpu/aerogpu_pci.ts";
import {
...
/**
 * Take an HDA stream descriptor through the reset handshake the Intel HDA spec requires.
 *
 * `SDnCTL.SRST` is not part of starting a stream. While it reads 1 the descriptor is held in
 * reset; the engine only becomes ready on the 1 → 0 edge, which is also where it clears the
 * stream position and any pending interrupt. Writing SRST and RUN together therefore parks the
 * descriptor in reset permanently — it never fetches a buffer descriptor and never moves a
 * sample. Callers should run this first, then program BDL/CBL/LVI/FMT, then set RUN on its own.
 */
async function resetHdaStreamDescriptor(readCtl: () => number, writeCtl: (value: number) => void): Promise<void> {
  const SRST = 1 << 0;
  const RUN = 1 << 1;
  // SDnCTL's top byte is SDnSTS, which is write-1-to-clear; keep our writes out of it.
  const CTL_MASK = 0x00ff_ffff;

  const waitForSrst = async (expected: number): Promise<void> => {
    const nowMs = (): number => (typeof performance?.now === "function" ? performance.now() : Date.now());
    const deadline = nowMs() + 1_000;
    while (((readCtl() & SRST) !== 0 ? SRST : 0) !== expected) {
      if (nowMs() >= deadline) {
        throw new Error(`Timed out waiting for HDA SDnCTL.SRST to read ${expected === 0 ? "0" : "1"}.`);
      }
      await sleepMs(1);
    }
  };

  // Assert reset, dropping RUN so the descriptor is not asked to run while it is resetting.
  writeCtl((((readCtl() & ~RUN) | SRST) & CTL_MASK) >>> 0);
  await waitForSrst(SRST);

  // Deassert. This edge is what readies the engine.
  writeCtl(((readCtl() & ~SRST) & CTL_MASK) >>> 0);
  await waitForSrst(0);
}
...
async function startHdaPciDevice(msg: AudioOutputHdaPciDeviceStartMessage, token: number): Promise<void> {
  ...
  await resetHdaStreamDescriptor(
    () => client.mmioRead(bar0Base + REG_SD0_CTL, 4) >>> 0,
    (value) => client.mmioWrite(bar0Base + REG_SD0_CTL, 4, value),
  );
  ...
  // RUN | IOCE | stream number 1. SRST stays clear: see `resetHdaStreamDescriptor`.
  const sdCtl = (SD_CTL_RUN | SD_CTL_IOCE | (1 << SD_CTL_STRM_SHIFT)) >>> 0;
...
async function startHdaCaptureSynthetic(msg: AudioHdaCaptureSyntheticStartMessage): Promise<void> {
  ...
  try {
    ...
    // RUN | stream number 2. SRST stays clear: see `resetHdaStreamDescriptor`.
    const SD_CTL_RUN = 1 << 1;
    const ctl = (SD_CTL_RUN | (2 << 20)) >>> 0;
