...
fn run_pf008_payload_32(payload: &[u8], expected_checksum: u32, scratch: Option<(u64, usize)>) {
    ...
    assert_eq!(cpu.state.gpr[Gpr::Rsp.as_u8() as usize], stack_top + 4);
}

#[test]
fn pf008_alu32_checksum() {
    // From the CPU and JIT area page (PF-008), `alu32` payload bytes.
    let payload: &[u8] = &[
...
fn pf008_call_ret32_checksum() {
    // From the CPU and JIT area page (PF-008), `call_ret32` payload bytes.
    let payload: &[u8] = &[
...
fn pf008_branch_pred32_checksum() {
    // From the CPU and JIT area page (PF-008), `branch_pred32` payload bytes.
    let payload: &[u8] = &[
...
fn pf008_mem_seq32_checksum() {
    // From the CPU and JIT area page (PF-008), `mem_seq32` payload bytes.
    let payload: &[u8] = &[
...
fn pf008_mem_stride32_checksum() {
    // From the CPU and JIT area page (PF-008), `mem_stride32` payload bytes.
    let payload: &[u8] = &[
        0xb8, 0xef, 0xcd, 0xab, 0x89, 0x31, 0xf6, 0x8b, 0x14, 0x37, 0x01, 0xd0, 0x31, 0xc2, 0x89,
        0x14, 0x37, 0x83, 0xc6, 0x40, 0x81, 0xe6, 0xff, 0x0f, 0x00, 0x00, 0x49, 0x75, 0xea, 0xc3,
    ];
    let scratch_base = 0xa000u64;
    run_pf008_payload_32(payload, 0x0da7_ebb4, Some((scratch_base, 4096)));
}

#[test]
fn pf008_branch_unpred32_checksum() {
    // From the CPU and JIT area page (PF-008), `branch_unpred32` payload bytes.
    let payload: &[u8] = &[
    ...
    run_pf008_payload_32(payload, 0xb1fd_f341, None);
}
