...
fn sort_tree(node: &mut KeyTreeNode) {
    node.children.sort_by_key(|c| c.name.to_uppercase());
    node.values.sort_by_key(|v| v.name.to_uppercase());
...
fn hive_to_tree(key: &RegistryKey<'_>) -> Result<KeyTreeNode> {
    ...
    values.sort_by_key(|v| v.name().to_uppercase());
    ...
    subkeys.sort_by_key(|k| k.name().to_uppercase());
