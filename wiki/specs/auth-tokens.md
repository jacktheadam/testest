...
> The token formats the gateway and proxies accept, and the strict verification
> contract every implementation must satisfy. Golden vectors under
> `protocol-vectors/` are the arbiter of correctness.
...
- **Gateway (TypeScript)**: `services/gateway/src/session.ts` (session cookie token), `services/gateway/src/udpRelay.ts` (UDP relay JWT minting)
...
- **Gateway public contract**: [`gateway-api.md`](gateway-api.md)
